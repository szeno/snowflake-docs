...
| **October 16, 2026** | Snowsight | [Snowsight: New account-specific host for sign-in traffic (Pending)](/release-notes/bcr-bundles/un-bundled/bcr-2413) | Snowsight moves sign-in and API traffic to an account-specific host. Organizations that filter outbound web traffic must allow the new `ACCOUNT_PUBLIC` host from `SYSTEM$ALLOWLIST()` in addition to the existing `apps-api` host, which must stay allowlisted. Private Link traffic isn’t affected. This change was originally announced in the 2026\_07 bundle and was removed from that bundle on September 3, 2026. |
| **Starting October 1, 2026** | Snowflake Native Apps | [Snowflake Native Apps: Bootstrap core Snowflake capabilities to native applications (Pending)](/release-notes/bcr-bundles/un-bundled/bcr-2437) | Installed and new native applications inherit a core set of public Snowflake capabilities through `SNOWFLAKE.APP_PUBLIC`, without a manifest request or a consumer grant. `SNOWFLAKE.APP_PUBLIC` is always active for an app and isn’t consumer-revocable. Account-level `PUBLIC` behavior is unchanged. Rolled out progressively. |
