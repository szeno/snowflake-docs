...
| Per-user quotas | Open | July 2026 | [Per-user quotas](/user-guide/budgets/per-user-quotas) | Enforce monthly and daily per-user credit limits on warehouse compute or AI domains, with built-in block enforcement, notifications, and Snowsight monitoring. |
| Backup instance types for compute pools | Open | July 2026 | [Backup instance types](/developer-guide/snowpark-container-services/working-with-compute-pool#label-spcs-working-with-compute-pool-backup-instance-types), [CREATE COMPUTE POOL](/sql-reference/sql/create-compute-pool), [ALTER COMPUTE POOL](/sql-reference/sql/alter-compute-pool) | Specify an ordered list of fallback instance families on a compute pool. When the primary instance type is capacity-constrained, Snowflake retries provisioning with the backup families automatically. |
