...
| --- | --- | --- | --- | --- |
| Account posture reporting in the Trust Center | Open | September 2026 | [Review account posture](/user-guide/trust-center/using-the-trust-center#label-trust-center-account-posture) | Review scanning status, open findings, and at-risk entities for your account over a selected period on the Trust Center Overview tab, and generate a PDF posture report covering scanner enablement, configuration changes, and violation and detection activity. |
| External secret providers | Open | September 2026 | [External secret providers](/user-guide/external-secret-providers), [Use external secret providers with Openflow](/user-guide/data-integration/openflow/security/external-secret-providers) | Access secrets from AWS Secrets Manager, Azure Key Vault, or Google Cloud Secret Manager through Snowflake and expose them as parameters for Openflow connectors. |
...
| [Container runtime for Streamlit apps in a Snowflake Native App](/developer-guide/native-apps/adding-streamlit) | Open | September 2026 | [Container runtime](/developer-guide/native-apps/adding-streamlit#label-streamlit-container-runtime-na) | Run a Streamlit app in a Snowflake Native App as a Snowpark Container Services service instead of on a virtual warehouse. Declared with the RUNTIME\_NAME parameter of CREATE STREAMLIT. Supports Python packages from PyPI, recent Streamlit library versions, and restricted caller’s rights. |
...
| Document generation in Snowflake CoWork | Open | August 2026 | [Document generation in Snowflake CoWork](/user-guide/snowflake-cortex/snowflake-cowork/document-generation) | Generate PDF documents, PowerPoint presentations, and HTML reports from your analysis, and view the generated file inline in Snowflake CoWork. Upload a PowerPoint template to match your organization’s style, or customize behavior with a user or agent skill. Requires the Cortex Agent [code execution tool](/user-guide/snowflake-cortex/cortex-agents-code-execution-tool). |
...
| Programmatic notifications for Trust Center findings | Open | April 2026 | [Programmatic notifications for Trust Center findings](/user-guide/trust-center/notification-integrations) | Send Trust Center findings to enterprise ticketing and monitoring systems through webhooks and queues such as PagerDuty, Slack, Microsoft Teams, Amazon SNS, Azure Event Grid, and Google Pub/Sub. Verify the integration immediately by sending a test notification. |
