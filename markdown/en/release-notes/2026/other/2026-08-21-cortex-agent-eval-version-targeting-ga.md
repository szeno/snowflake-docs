...
Version targeting pins the configuration under evaluation, not the agent behavior that gets evaluated: each run invokes the agent again and scores a newly generated trace. For how to read scores that move between runs, see [Interpret score variance across runs](/user-guide/snowflake-cortex/cortex-agents-evaluations#label-agent-evaluation-score-variance).
