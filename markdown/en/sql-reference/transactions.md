...
![Illustration of two stored procedures, each with its own scoped transaction.](/static/images/scoped-transactions-2-layer-diagram-04.png)
...
![Illustration of two stored procedures and two scoped transactions, in which one transaction includes some statements from the enclosed stored procedure as well as all statements from the enclosing stored procedure.](/static/images/scoped-transactions-2-layer-diagram-06.png)
...
![Illustration of two stored procedures that create improperly-scoped transactions.](/static/images/scoped-transactions-improperly-placed-commit-diagram-09.png)
...
small delay in query response times (usually milliseconds), set the [READ\_CONSISTENCY\_MODE](/sql-reference/parameters#label-read-consistency-mode) parameter to `'GLOBAL'`. By setting
...
3. Use the [ALTER ACCOUNT](/sql-reference/sql/alter-account) command to set READ\_CONSISTENCY\_MODE to `'GLOBAL'`:
...
Two parameters control timeout for locks: [LOCK\_TIMEOUT](/sql-reference/parameters#label-lock-timeout) and [HYBRID\_TABLE\_LOCK\_TIMEOUT](/sql-reference/parameters#label-hybrid-table-lock-timeout).

#### LOCK\_TIMEOUT parameter
...
#### HYBRID\_TABLE\_LOCK\_TIMEOUT parameter
...
The [LOCK\_WAIT\_HISTORY view](/sql-reference/account-usage/lock_wait_history) logs a detailed history of transactions with respect
...
You can call the following function to abort a transaction: [SYSTEM$ABORT\_TRANSACTION](/sql-reference/functions/system_abort_transaction).
...
To abort a running transaction, the user who started the transaction or an account administrator can call the system function, [SYSTEM$ABORT\_TRANSACTION](/sql-reference/functions/system_abort_transaction).
...
To allow a statement error within a transaction to abort the transaction, set the [TRANSACTION\_ABORT\_ON\_ERROR](/sql-reference/parameters#label-transaction-abort-on-error) parameter at the session or account level.

### Analyzing blocked transactions with the LOCK\_WAIT\_HISTORY view

The [LOCK\_WAIT\_HISTORY view](/sql-reference/account-usage/lock_wait_history) returns transaction details that can be useful in analyzing blocked transactions.
...
![Example of blocked and blocker transactions.](/static/images/blocked-transaction.png)
...
1. Query the Account Usage [QUERY\_HISTORY view](/sql-reference/account-usage/query_history) for transactions that waited for locks in the last 24 hours:
...
You can also use the [CURRENT\_TRANSACTION](/sql-reference/functions/current_transaction) function to return the transaction ID of the transaction currently running in the session.
...
In the [LOCK\_WAIT\_HISTORY view](/sql-reference/account-usage/lock_wait_history), the output behaves as follows:
