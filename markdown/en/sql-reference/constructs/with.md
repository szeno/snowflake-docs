...
The WITH clause is an optional clause that precedes the body of the [SELECT](/sql-reference/sql/select) statement, and defines one
or more [CTEs (common table expressions)](/user-guide/queries-cte) that can be used later in the statement. For example,
CTEs can be referenced in the [FROM](/sql-reference/constructs/from) clause.
...
a CALL command rather than a SELECT command. For more information, see [CALL (with anonymous procedure)](/sql-reference/sql/call-with).

The WITH clause is used with machine learning model objects to create an alias to a specific version of the model,
which can then be used to call the methods of that version. See [Calling model methods](/sql-reference/commands-model-function#label-snowpark-model-registry-model-methods).

See also:
:   [CONNECT BY](/sql-reference/constructs/connect-by), [Model commands](/sql-reference/commands-model)
...
`cte_name1` , `cte_nameN`
:   The CTE name must follow the rules for views and similar [object identifiers](/sql-reference/identifiers).
...
[STATEMENT\_TIMEOUT\_IN\_SECONDS](/sql-reference/parameters#label-statement-timeout-in-seconds) parameter), or you [cancel the query](/user-guide/querying-cancel-statements).
...
The anchor clause in a recursive CTE is a [SELECT](/sql-reference/sql/select) statement.
...
The recursive clause is a [SELECT](/sql-reference/sql/select) statement. This SELECT is restricted to projections, filters, and
