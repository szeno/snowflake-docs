...
The AT or BEFORE clause is used for Snowflake Time Travel. In a query, it is specified in the [FROM](/sql-reference/constructs/from) clause
...
You can use the same syntax to clone objects; see [CREATE <object> … CLONE](/sql-reference/sql/create-clone). If you don’t
...
:   Specifies the identifier (i.e. name) for an existing stream on the queried table or view. The current offset in
    ...
    This keyword is supported only when creating a stream (using [CREATE STREAM](/sql-reference/sql/create-stream)) or querying change data (using
    the [CHANGES](/sql-reference/constructs/changes) clause). For examples, see those topics.
...
- The AT and BEFORE clauses do not support selecting historical data from a [CTE](/user-guide/queries-cte).
  ...
  However, these clauses are supported in a query in a [WITH](/sql-reference/constructs/with) clause. For example, the following
  query is supported:
...
- CREATE DATABASE … CLONE and CREATE SCHEMA … CLONE commands that use Time Travel and specify the time with the STATEMENT parameter
  ...
  [IGNORE HYBRID TABLES parameter](/sql-reference/sql/create-clone#label-ignore-hybrid-tables-parameter). When you include this parameter, the command will
...
| Solution | Cast the string to a timestamp.  Copy code  ``` ... AT(TIMESTAMP => '2018-07-27 12:00:00')               -- fails ... AT(TIMESTAMP => '2018-07-27 12:00:00'::TIMESTAMP)    -- succeeds ``` |
...
Substitute a recent date, time, or timestamp that’s within the retention period.
