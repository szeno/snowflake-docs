...
[COPY INTO <table>](/sql-reference/sql/copy-into-table) command. The view displays one row for each file loaded.

Note

This view does not return the history of data loaded using Snowpipe. For this historical information, query
the [COPY\_HISTORY](/sql-reference/organization-usage/copy_history) view instead.
...
- After the replication of load history, the LOAD\_HISTORY Organization Usage view shows the history only after the latest truncate operation
