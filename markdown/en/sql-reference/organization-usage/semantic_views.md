...
This Organization Usage view displays a row for each [semantic view](/user-guide/views-semantic/overview) in an account.
...
| DELETED | TIMESTAMP\_LTZ | Date and time when the semantic view was dropped. |
| COMMENT | VARCHAR | Comment for the semantic view. |
| MAX\_STALESNESS\_SEC | NUMBER | Maximum staleness in seconds allowed for the semantic view materialization. NULL if no materialization target lag is set. |
