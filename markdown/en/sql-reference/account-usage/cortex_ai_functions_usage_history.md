...
This Account Usage view can be used to query the usage history of [Cortex AI Functions](/user-guide/snowflake-cortex/aisql).

The view includes the number of tokens and credits consumed each time a Cortex AI Function is called, aggregated into one-hour windows. The view also includes relevant metadata, such as the warehouse ID, start and end times of the function
...
| --- | --- | --- |
| START\_TIME | TIMESTAMP\_LTZ | Start of the usage aggregation window. The window resolution is 1 hour. For example, if a query began at 5:30 and completed at 8:30, four records appear in the usage view, one each for the 5:00, 6:00, 7:00, and 8:00 aggregation windows. |
...
- Running queries are updated every 2 minutes (best effort) with an SLA of 5 minutes.
- The credit usage rate is determined based on the function called, model used, and the tokens processed as outlined in the
