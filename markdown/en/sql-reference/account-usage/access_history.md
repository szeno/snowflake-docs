...
| objectId | NUMBER | An identifier for the object, which is unique within a given account and domain. This number will match:   - The `TABLE_ID` number for a table, view, or materialized view. You can obtain this value from [tables](/sql-reference/account-usage/tables), [views](/sql-reference/account-usage/views), or [materialized view refresh history](/sql-reference/account-usage/materialized_view_refresh_history). - If a stage was accessed, this number will match the:   - `NAME` identifier for a user stage (see [users](/sql-reference/account-usage/users))   - `TABLE_ID` number for a table stage (see [tables](/sql-reference/account-usage/tables))   - `STAGE_ID` number for a name stage (see [stages](/sql-reference/account-usage/stages)) |
...
:   Short-running queries that operate exclusively against hybrid tables will no
    longer generate a record in the QUERY\_HISTORY view, in [query history](/sql-reference/account-usage/query_history), or
...
The `ACCESS_HISTORY` view also captures successful operations that external query engines
(such as Spark, Trino, DuckDB, or PyIceberg) perform against Snowflake-managed
Apache Iceberg™ tables through the Horizon Iceberg REST Catalog (IRC) APIs. You can identify
these records by filtering on `event_source = 'horizon_irc'`, and inspect the
`additional_properties` column to see the Iceberg operation type. For details, see
