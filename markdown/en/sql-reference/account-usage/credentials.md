...
[second factor](/user-guide/security-mfa-second-factor) for authentication. This view includes rows for the following types
of credentials:
...
This view does not include information about [Duo authenticators](/user-guide/security-mfa-duo) (Duo push and passcodes).

To determine if a user has configured Duo as a second factor for authentication, you can run the
[SHOW MFA METHODS](/sql-reference/sql/show-mfa-methods) command.
...
The following example returns rows for [programmatic access tokens](/user-guide/programmatic-access-tokens):
