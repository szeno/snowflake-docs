...
| REINIT\_REASON | VARCHAR | When REFRESH\_ACTION is REINITIALIZE, describes why the dynamic table was reinitialized, such as base table changes or adaptive refresh mode decisions. NULL for other refresh actions. |
...
- To query this view, use a role that is granted the SNOWFLAKE.USAGE\_VIEWER [database role](/sql-reference/snowflake-db-roles).
