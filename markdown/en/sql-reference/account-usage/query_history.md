...
| `query_type` | VARCHAR | DML, query, and so on. If the query failed, then the query type may be UNKNOWN. |  |
...
| `user_type` | VARCHAR | The type of the user executing the query. It’s the same as the `type` column in the [users](/sql-reference/account-usage/users). If a Snowpark Container Services service executes the query, the user type is SNOWFLAKE\_SERVICE. |  |
