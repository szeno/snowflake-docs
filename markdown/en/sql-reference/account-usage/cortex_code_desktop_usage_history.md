...
The CORTEX\_CODE\_DESKTOP\_USAGE\_HISTORY view can be used to query the usage history of
[CoCo Desktop](/user-guide/cortex-code/cortex-code).

The information in the view includes the number of credits consumed each time a
user interacts with CoCo Desktop. Each row in the view represents a single
...
This view does not include requests originating from CoCo CLI or CoCo in Snowsight.
Requests originating from CoCo CLI are recorded in the
[CORTEX\_CODE\_CLI\_USAGE\_HISTORY](/sql-reference/account-usage/cortex_code_cli_usage_history) view.
Requests originating from CoCo in Snowsight are recorded in the
[CORTEX\_CODE\_SNOWSIGHT\_USAGE\_HISTORY](/sql-reference/account-usage/cortex_code_snowsight_usage_history) view.

To query usage across CoCo CLI, Desktop, and Snowsight in a single view, use
[SNOWFLAKE\_COCO\_USAGE\_HISTORY](/sql-reference/account-usage/snowflake_coco_usage_history).
...
- This view is part of the `SNOWFLAKE.ACCOUNT_USAGE` schema. To enable access for roles other than ACCOUNTADMIN, follow the steps in [Enabling other roles to use the ACCOUNT\_USAGE schema](/sql-reference/account-usage#label-enabling-usage-for-other-roles).
...
Retrieve CoCo Desktop usage history:
