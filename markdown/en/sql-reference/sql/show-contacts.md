...
Lists the [contacts](/user-guide/contacts-using) for which you have access privileges.
...
See also:
:   [CREATE CONTACT](/sql-reference/sql/create-contact) , [ALTER CONTACT](/sql-reference/sql/alter-contact) , [DROP CONTACT](/sql-reference/sql/drop-contact)
...
| `owner_role_type` | The type of role that has OWNERSHIP privilege on the contact. Either ROLE, DATABASE\_ROLE, or APPLICATION (if a Snowflake Native App owns the object). |
