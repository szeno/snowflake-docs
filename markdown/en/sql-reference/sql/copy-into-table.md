...
:   Specifies the internal or external location where the files containing data to be loaded are staged:
    ...
    >
    >     Note that currently, accessing Azure blob storage in [government regions](/user-guide/intro-regions#label-us-gov-regions) using a storage
    ...
    >   ```
    >
    >   When you specify a path in COPY INTO *<table>* statements, Snowflake treats the path value as a prefix and performs a prefix match against the file name in the external location. In the previous examples, Snowflake would load all files with a name that starts with `./../a.csv` in the external location.
    >
    > Note
    >
...
:   Supported when the FROM value in the COPY statement is an external storage URI rather than an external stage name.Required only for loading from an external private/protected cloud storage location; not required for public buckets/containers
    ...
    **Amazon S3**
    ...
    >
    >       If you must use permanent credentials, use [external stages](/sql-reference/sql/create-stage), for which credentials are
...
:   Required for transforming data during loading
    ...
    the files using a standard SQL query (i.e. [SELECT](/sql-reference/sql/select) list), where:
...
:   Specifies the format of the data files to load:
    ...
    :   Specifies an existing named file format to use for loading data into the table. The named file format determines the format type
        (CSV, JSON, etc.), as well as any other format options, for the data files. For more information, see [CREATE FILE FORMAT](/sql-reference/sql/create-file-format).
...
`DATE_FORMAT = 'string' | AUTO`
:   String that defines the format of date values in the data files to be loaded. If a value is not specified or is `AUTO`, the value for the [DATE\_INPUT\_FORMAT](/sql-reference/parameters#label-date-input-format) session parameter is used.

    Default:
    :   `AUTO`

`TIME_FORMAT = 'string' | AUTO`
:   String that defines the format of time values in the data files to be loaded. If a value is not specified or is `AUTO`, the value for the [TIME\_INPUT\_FORMAT](/sql-reference/parameters#label-time-input-format) session parameter is used.

    Default:
    :   `AUTO`

`TIMESTAMP_FORMAT = 'string' | AUTO`
:   String that defines the format of timestamp values in the data files to be loaded. If a value is not specified or is `AUTO`, the value for the [TIMESTAMP\_INPUT\_FORMAT](/sql-reference/parameters#label-timestamp-input-format) session parameter
    is used.
...
:   Boolean that specifies whether to generate a parsing error if the number of delimited columns (that is, fields) in an input data file does not match the number of columns in the corresponding table.
    ...
    When [transforming data during loading](/user-guide/data-load-transform) (that is, using a query as the source for the [COPY INTO <table>](/sql-reference/sql/copy-into-table) command), this option is ignored. There is no requirement for your data files
...
:   Definition:
    ...
    For additional usage notes, see the [LOAD\_MODE usage notes](/sql-reference/sql/copy-into-table#label-copy-into-table-usage-notes-iceberg-parquet).
    For examples, see [Loading Iceberg-compatible Parquet data into an Iceberg table](/sql-reference/sql/copy-into-table#label-copy-into-table-examples-iceberg-compatible-parquet).
...
:   Use:
    ...
    :   - `CONTINUE`
        ...
        - `ABORT_STATEMENT`
          ...
          The terminated operations don’t show up in [COPY\_HISTORY](/sql-reference/functions/copy_history) as the data files weren’t ingested. We recommend that you search for the failures in [QUERY\_HISTORY](/sql-reference/functions/query_history).
...
- If you encounter errors while running the COPY command, after the command completes, you can validate the files that produced the errors
  using the [VALIDATE](/sql-reference/functions/validate) table function.
...
Load files from a named external stage that you created previously using the [CREATE STAGE](/sql-reference/sql/create-stage) command. The named
