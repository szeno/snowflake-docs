# DROP *<object>*

Removes the specified object from the system.

See also:
:   [CREATE <object>](/sql-reference/sql/create) , [SHOW <objects>](/sql-reference/sql/show) , [UNDROP <object>](/sql-reference/sql/undrop)

## DROP commands

For specific syntax, usage notes, and examples, see:

**Organization Objects:**

- [DROP ACCOUNT](/sql-reference/sql/drop-account)

**Account Objects:**

- [DROP APPLICATION](/sql-reference/sql/drop-application)
- [DROP APPLICATION PACKAGE](/sql-reference/sql/drop-application-package)
- [DROP AUTHENTICATION POLICY](/sql-reference/sql/drop-authentication-policy)
- [DROP CATALOG INTEGRATION](/sql-reference/sql/drop-catalog-integration)
- [DROP COMPUTE POOL](/sql-reference/sql/drop-compute-pool)
- [DROP CONNECTION](/sql-reference/sql/drop-connection)
- [DROP DATABASE](/sql-reference/sql/drop-database)
- [DROP DATABASE ROLE](/sql-reference/sql/drop-database-role)
- [DROP EXTERNAL VOLUME](/sql-reference/sql/drop-external-volume)
- [DROP FAILOVER GROUP](/sql-reference/sql/drop-failover-group)
- [DROP FEATURE POLICY](/sql-reference/sql/drop-feature-policy)
- [DROP INTEGRATION](/sql-reference/sql/drop-integration)
- [DROP NETWORK POLICY](/sql-reference/sql/drop-network-policy)
- [DROP ORGANIZATION PROFILE](/sql-reference/sql/drop-organization-profile)
- [DROP POSTGRES INSTANCE](/sql-reference/sql/drop-postgres-instance)
- [DROP REPLICATION GROUP](/sql-reference/sql/drop-replication-group)
- [DROP RESOURCE MONITOR](/sql-reference/sql/drop-resource-monitor)
- [DROP ROLE](/sql-reference/sql/drop-role)
- [DROP SHARE](/sql-reference/sql/drop-share)
- [DROP USER](/sql-reference/sql/drop-user)
- [DROP WAREHOUSE](/sql-reference/sql/drop-warehouse)

**Database Objects:**

- [DROP AGENT](/sql-reference/sql/drop-agent)
- [DROP AGGREGATION POLICY](/sql-reference/sql/drop-aggregation-policy)
- [DROP ALERT](/sql-reference/sql/drop-alert)
- [DROP AUTHENTICATION POLICY](/sql-reference/sql/drop-authentication-policy)
- [DROP BACKUP POLICY](/sql-reference/sql/drop-backup-policy)
- [DROP BACKUP SET](/sql-reference/sql/drop-backup-set)
- [DROP CONTACT](/sql-reference/sql/drop-contact)
- [DROP CORTEX SEARCH SERVICE](/sql-reference/sql/drop-cortex-search)
- [DROP DATA MOVEMENT POLICY](/sql-reference/sql/drop-data-movement-policy)
- [DROP DATA MOVEMENT RULE](/sql-reference/sql/drop-data-movement-rule)
- [DROP DBT PROJECT](/sql-reference/sql/drop-dbt-project)
- [DROP DCM PROJECT](/sql-reference/sql/drop-dcm-project)
- [DROP DYNAMIC TABLE](/sql-reference/sql/drop-dynamic-table)
- [DROP EXPERIMENT](/sql-reference/sql/drop-experiment)
- [DROP EXTERNAL AGENT](/sql-reference/sql/drop-external-agent)
- [DROP EXTERNAL TABLE](/sql-reference/sql/drop-external-table)
- [DROP FILE FORMAT](/sql-reference/sql/drop-file-format)
- [DROP FUNCTION](/sql-reference/sql/drop-function)
- [DROP GATEWAY](/sql-reference/sql/drop-gateway)
- [DROP GIT REPOSITORY](/sql-reference/sql/drop-git-repository)
- [DROP ICEBERG TABLE](/sql-reference/sql/drop-iceberg-table)
- [DROP IMAGE REPOSITORY](/sql-reference/sql/drop-image-repository)
- [DROP INDEX](/sql-reference/sql/drop-index)
- [DROP JOIN POLICY](/sql-reference/sql/drop-join-policy)
- [DROP LISTING](/sql-reference/sql/drop-listing)
- [DROP MAINTENANCE POLICY](/sql-reference/sql/drop-maintenance-policy)
- [DROP MASKING POLICY](/sql-reference/sql/drop-masking-policy)
- [DROP MATERIALIZED VIEW](/sql-reference/sql/drop-materialized-view)
- [DROP MCP SERVER](/sql-reference/sql/drop-mcp-server)
- [DROP MODEL](/sql-reference/sql/drop-model)
- [DROP MODEL MONITOR](/sql-reference/sql/drop-model-monitor)
- [DROP NETWORK RULE](/sql-reference/sql/drop-network-rule)
- [DROP NOTEBOOK](/sql-reference/sql/drop-notebook)
- [DROP ONLINE FEATURE TABLE](/sql-reference/sql/drop-online-feature-table)
- [DROP PACKAGES POLICY](/sql-reference/sql/drop-packages-policy)
- [DROP PASSWORD POLICY](/sql-reference/sql/drop-password-policy)
- [DROP PIPE](/sql-reference/sql/drop-pipe)
- [DROP PRIVACY POLICY](/sql-reference/sql/drop-privacy-policy)
- [DROP PROCEDURE](/sql-reference/sql/drop-procedure)
- [DROP PROJECTION POLICY](/sql-reference/sql/drop-projection-policy)
- [DROP RESTRICTED SESSION SCOPE](/sql-reference/sql/drop-restricted-session-scope)
- [DROP ROW ACCESS POLICY](/sql-reference/sql/drop-row-access-policy)
- [DROP SCHEMA](/sql-reference/sql/drop-schema)
- [DROP SECRET](/sql-reference/sql/drop-secret)
- [DROP SEMANTIC VIEW](/sql-reference/sql/drop-semantic-view)
- [DROP SEQUENCE](/sql-reference/sql/drop-sequence)
- [DROP SERVICE](/sql-reference/sql/drop-service)
- [DROP SESSION POLICY](/sql-reference/sql/drop-session-policy)
- [DROP SNAPSHOT](/sql-reference/sql/drop-snapshot)
- [DROP SNAPSHOT POLICY — Deprecated](/sql-reference/sql/drop-snapshot-policy) (deprecated; prefer [DROP BACKUP POLICY](/sql-reference/sql/drop-backup-policy))
- [DROP SNAPSHOT SET — Deprecated](/sql-reference/sql/drop-snapshot-set) (deprecated; prefer [DROP BACKUP SET](/sql-reference/sql/drop-backup-set))
- [DROP STAGE](/sql-reference/sql/drop-stage)
- [DROP STORAGE LIFECYCLE POLICY](/sql-reference/sql/drop-storage-lifecycle-policy)
- [DROP STREAM](/sql-reference/sql/drop-stream)
- [DROP STREAMLIT](/sql-reference/sql/drop-streamlit)
- [DROP TABLE](/sql-reference/sql/drop-table)
- [DROP TAG](/sql-reference/sql/drop-tag)
- [DROP TASK](/sql-reference/sql/drop-task)
- [DROP TYPE](/sql-reference/sql/drop-type)
- [DROP VIEW](/sql-reference/sql/drop-view)

**Classes:**

- [DROP SNOWFLAKE.ML.ANOMALY\_DETECTION](/sql-reference/classes/anomaly-detection/commands/drop-anomaly-detection#label-class-anomaly-detection-drop)
- [DROP BUDGET](/sql-reference/classes/budget/commands/drop-budget)
- [DROP CLASSIFICATION\_PROFILE](/sql-reference/classes/classification_profile/commands/drop-classification-profile)
- [DROP CUSTOM\_CLASSIFIER](/sql-reference/classes/custom_classifier/commands/drop-custom-classifier)
- [DROP SNOWFLAKE.ML.FORECAST](/sql-reference/classes/forecast/commands/drop-forecast#label-class-forecast-drop)
