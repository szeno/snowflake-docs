...
:   Specifies the external stage and optional path where the files containing data to be read are staged:
    ...
    > - `<path>` is an optional case-sensitive directory path for files in the cloud storage location that limits the set of files to load. Paths are alternatively called *prefixes* or *folders* by different cloud storage services.
    >
    ...
    > Note
    >
    > - Specify a full *directory* path, and not a partial path (shared prefix) for files in your storage location (for example, use a path like `@my_ext_stage/2025/` instead of `@my_ext_stage/2025-*`). To filter for files that share a common prefix, use partition columns instead.
...
:   String that defines an inline or out-of-line constraint for the specified columns in the table.
    ...
    For syntax details, see [CREATE | ALTER TABLE … CONSTRAINT](/sql-reference/sql/create-table-constraint). For more information about constraints, see
...
:   Specifies whether to automatically refresh the external table metadata one time, immediately after the external table is created. Refreshing
    ...
    :   Snowflake doesn’t automatically refresh the external table metadata. To register any existing data files in the stage, you must
        manually refresh the external table metadata one time by using [ALTER EXTERNAL TABLE](/sql-reference/sql/alter-external-table) … REFRESH.
...
:   Specifies whether Snowflake should enable triggering automatic refreshes of the external table metadata when new or updated data
    ...
    - Setting this parameter to TRUE isn’t supported for external tables that reference data files
      ...
      that provides an API compliant with the S3 REST API). For more information, see [Work with Amazon S3-compatible storage](/user-guide/data-load-s3-compatible-storage).
    ...
    :   Snowflake doesn’t enable triggering automatic refreshes of the external table metadata. You must manually refresh the external table
        metadata periodically by using [ALTER EXTERNAL TABLE](/sql-reference/sql/alter-external-table) … REFRESH to synchronize the metadata with the current list of files in the
        stage path.
...
>
>   Periodically run an [ALTER EXTERNAL TABLE … REFRESH](/sql-reference/sql/alter-external-table) statement to register any
...
:   Associates one or more [data metric functions (DMFs)](/user-guide/data-quality-intro) with the external table at creation time.
...
- Using `OR REPLACE` is the equivalent of using [DROP EXTERNAL TABLE](/sql-reference/sql/drop-external-table) on the existing external table, and then creating a new
...
- When you create an external table with a row access policy added to the external table, use the
  [POLICY\_CONTEXT](/sql-reference/functions/policy_context) function to simulate a query on the external table that is protected by a row access policy.
...
3. Create the partitioned external table.
   ...
   [TO\_DATE , DATE](/sql-reference/functions/to_date). The SQL command also specifies Parquet as the file format type.
...
2. Create the partitioned external table. The external table includes three partition columns with different data types.
   ...
   - A partition column name must be in uppercase, unless the column name is enclosed in double quotes. Alternatively,
     use [GET\_IGNORE\_CASE](/sql-reference/functions/get_ignore_case) instead of the case-sensitive `:` character in the SQL
     expression.
...
The following example builds on an example in the [INFER\_SCHEMA](/sql-reference/functions/infer_schema) topic:
