...
See also:
:   [ALTER TABLE](/sql-reference/sql/alter-table) , [DROP TABLE](/sql-reference/sql/drop-table) , [SHOW TABLES](/sql-reference/sql/show-tables) , [DESCRIBE TABLE](/sql-reference/sql/desc-table)
...
> `col_name` is an [object identifier](/sql-reference/identifiers). It must follow the [requirements for Snowflake identifiers](/sql-reference/identifiers-syntax).
>
> `col_type` is one of the [Snowflake data types](/sql-reference-data-types), such as
> [NUMBER](/sql-reference/data-types-numeric#label-data-type-number) or [VARCHAR](/sql-reference/data-types-text#label-data-types-text-varchar).
>
> `AS ( expr )` defines the column as a virtual column. Snowflake computes the column’s values from the expression at query time. For ANSI SQL compatibility, you can also use the equivalent `[ GENERATED ALWAYS ] AS ( expr ) [ VIRTUAL ]` form. When you specify `AS`, the `NOT NULL` and `DEFAULT` clauses, and `CHECK` constraints, can’t be applied to the same column. For the full list of allowed expressions, data type rules, and restrictions, see [Virtual Columns](/sql-reference/virtual-columns).
>
> Copy code
>
> ```
...
> ```
>
> For additional out-of-line constraint details, see [CREATE | ALTER TABLE … CONSTRAINT](/sql-reference/sql/create-table-constraint).
...
For more information, see [CREATE OR ALTER TABLE usage notes](#label-create-or-alter-table-usage-notes) and [CREATE OR ALTER <object>](/sql-reference/sql/create-or-alter).
...
For more details about COPY GRANTS, see [COPY GRANTS](/sql-reference/sql/create-table#label-create-table-copy-grants) in this document.

### CREATE TABLE … USING TEMPLATE

Creates a new table with the column definitions derived from a set of staged files using the [INFER\_SCHEMA](/sql-reference/functions/infer_schema) function.
...
For details about cloning dynamic tables to tables, see [Clone a dynamic table to a regular table](/user-guide/dynamic-tables/cloning#label-clone-dt-to-table).
...
the [Usage notes](/sql-reference/sql/create-table#label-create-table-archive-of-notes).
...
:   Required for CTAS and USING TEMPLATE.
    ...
    - For CTAS, specifies the [SELECT statement](/sql-reference/constructs) that populates the table. This query must be
      specified last in the CTAS statement, regardless of the other parameters that you include.
    - For CREATE TABLE … USING TEMPLATE, specifies the subquery that calls the [INFER\_SCHEMA](/sql-reference/functions/infer_schema) function and
...
:   Specifies a required condition that filters the archived rows to retrieve. The expression can reference columns
    ...
    For more information about WHERE expressions, see [WHERE](/sql-reference/constructs/where).
...
:   Specifies that the table persists only for the duration of the [session](/user-guide/session-policies) that you created it in. A
    ...
    For a temporary table that exists only for a single [Snowflake Scripting](/developer-guide/snowflake-scripting/index) stored procedure
    execution, use `CREATE OR REPLACE PROCEDURE SCOPED TEMP TABLE` inside the procedure body instead of
    `CREATE TEMPORARY TABLE`. See [Procedure-scoped temporary tables (Snowflake Scripting)](/sql-reference/sql/create-procedure#label-create-procedure-scoped-temp-table).
...
:   Specifies the collation to use for column operations such as string comparisons. This parameter applies only to
    [text columns](/sql-reference/data-types-text#label-character-datatypes): VARCHAR, STRING, TEXT, and so on. For more information,
    see [Collation specifications](/sql-reference/collation#label-collation-specification).
...
:   Specifies whether a default value is automatically inserted in the column if a value is not explicitly specified via an INSERT
    ...
    >
    >     - Constant value.
    >     - [Sequence reference](/user-guide/querying-sequences#label-sequence-default-column-values) (`seq_name.NEXTVAL`).
    ...
    >     see [Sequence Semantics](/user-guide/querying-sequences#label-sequence-semantics).
    >
    ...
    >
    >     If you do not specify ORDER or NOORDER, the [NOORDER\_SEQUENCE\_AS\_DEFAULT](/sql-reference/parameters#label-noorder-sequence-as-default) parameter determines which property is
    >     set.
...
`DEFAULT_DDL_COLLATION = 'collation_specification'`
:   Specifies a default [collation specification](/sql-reference/collation#label-collation-specification) for the columns in the table, including columns
...
`COPY TAGS`
:   Applies [tags](/user-guide/object-tagging/introduction) when you use any of these CREATE TABLE forms:
    ...
    [the usage notes for COPY TAGS](/sql-reference/sql/create-table#label-create-table-usage-notes-copy-tags).
...
- A schema cannot contain tables and/or views with the same name. When creating a table:
  ...
  >   Important
  >
  >   Using `OR REPLACE` is the equivalent of using [DROP TABLE](/sql-reference/sql/drop-table) on the existing table and then creating a new table
  ...
  >
  >   Recreating or swapping a table drops its change data. Any stream on the table becomes [stale](/user-guide/streams-intro#label-streams-staleness). In
  >   addition, any stream on a view that has this table as an underlying table, becomes stale. A stale stream is unreadable.
- The definition for a table is limited to 1 MB.
- The `OR REPLACE` and `IF NOT EXISTS` clauses are mutually exclusive. They can’t both be used in the same statement.
- Similar to [reserved keywords](/sql-reference/reserved-keywords), ANSI-reserved function names
  ([CURRENT\_DATE](/sql-reference/functions/current_date), [CURRENT\_TIMESTAMP](/sql-reference/functions/current_timestamp), etc.) cannot be used as column
  names.
- CREATE OR ALTER TABLE:
...
- CREATE TABLE … AS SELECT (CTAS):
  ...
  - If the aliases for the column names in the [SELECT](/sql-reference/sql/select) list are valid columns, then the column definitions
    ...
    The number of column names specified must match the number of [SELECT](/sql-reference/sql/select) list items in the query; the
...
- **Limitations**
  ...
  - Currently only supports permanent, temporary, transient, and dynamic tables. Read-only, external, Apache Iceberg™, and hybrid tables
    are *not* supported. For dynamic tables, see [CREATE OR ALTER DYNAMIC TABLE](/sql-reference/sql/create-dynamic-table#label-create-or-alter-dt-syntax).
  - All limitations of the [ALTER TABLE](/sql-reference/sql/alter-table) command apply.
...
- **Table parameters and properties**
  ...
  - Unsetting an explicit [parameter](/sql-reference/parameters) value results in setting it to the default parameter value.
...
- **Columns**
  ...
  - For more information about modifying columns, see [ALTER TABLE … ALTER COLUMN](/sql-reference/sql/alter-table-column).
- **Collation**

  - Collation specifications cannot be altered.
  - Setting the [DEFAULT\_DDL\_COLLATION](/sql-reference/parameters#label-default-ddl-collation) parameter in the CREATE OR ALTER TABLE command
...
- **Atomicity**
  ...
  The CREATE OR ALTER TABLE command does not guarantee atomicity. In the very rare case that a CREATE OR ALTER TABLE statement
  for an extremely wide table fails during execution, it is possible that a subset of changes might have been applied to the table. If there is a possibility
...
It builds on an example in the [INFER\_SCHEMA](/sql-reference/functions/infer_schema) topic.
