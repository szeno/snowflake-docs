...
:   Specifies the source of the data to be unloaded, which can either be a table or a query:
    ...
    >
    > `( query )`
    > :   [SELECT](/sql-reference/sql/select) statement that returns data to be unloaded into files. You can limit the number of rows returned by specifying a
    >     [LIMIT / FETCH](/sql-reference/constructs/limit) clause in the query.
    >
    >     Note
    >
    >     When casting column values to a data type using the [CAST , ::](/sql-reference/functions/cast) function, verify the data type supports
...
:   Supported when the COPY statement specifies an external storage URI rather than an external stage name for the target cloud storage location. Specifies the security credentials for connecting to the cloud provider and accessing the private storage container where the unloaded files are staged.
    ...
    **Amazon S3**
    ...
    >
    >       If you must use permanent credentials, use [external stages](/sql-reference/sql/create-stage), for which credentials are entered
...
:   Specifies an expression used to partition the unloaded table rows into separate files. Supports any SQL expression that evaluates to a
    ...
    - [PREVENT\_UNLOAD\_TO\_INLINE\_URL](/sql-reference/parameters#label-prevent-unload-to-inline-url) prevents ad hoc data unload operations to external cloud storage locations (i.e. COPY INTO
      *<location>* statements that specify the cloud storage URL and access settings directly in the statement).
    - [PREVENT\_UNLOAD\_TO\_INTERNAL\_STAGES](/sql-reference/parameters#label-prevent-unload-to-internal-stages) prevents data unload operations to any internal stage, including user stages,
    ...
    - When unloading to files of type `PARQUET`:
      ...
      - VARIANT columns are converted into simple JSON strings. Casting the values to an array (using the
        [TO\_ARRAY](/sql-reference/functions/to_array) function) results in an array of JSON strings.
...
:   Specifies the format of the data files containing unloaded data:
    ...
    :   Specifies an existing named file format to use for unloading data from the table. The named file format determines the format type
        (CSV, JSON, PARQUET), as well as any other format options, for the data files. For more information, see [CREATE FILE FORMAT](/sql-reference/sql/create-file-format).
...
`DATE_FORMAT = 'string' | AUTO`
:   String that defines the format of date values in the unloaded data files. If a value is not specified or is set to `AUTO`, the value for the [DATE\_OUTPUT\_FORMAT](/sql-reference/parameters#label-date-output-format) parameter is used.

    Default: `AUTO`

`TIME_FORMAT = 'string' | AUTO`
:   String that defines the format of time values in the unloaded data files. If a value is not specified or is set to `AUTO`, the value for the [TIME\_OUTPUT\_FORMAT](/sql-reference/parameters#label-time-output-format) parameter is used.

    Default: `AUTO`

`TIMESTAMP_FORMAT = 'string' | AUTO`
:   String that defines the format of timestamp values in the unloaded data files. If a value is not specified or is set to `AUTO`, the value for the [TIMESTAMP\_OUTPUT\_FORMAT](/sql-reference/parameters#label-timestamp-output-format) parameter is used.
...
- When unloading to files of type `CSV`, `JSON`, or `PARQUET`:
  ...
  - To unload the data as Parquet LIST values, explicitly cast the column values to arrays
    (using the [TO\_ARRAY](/sql-reference/functions/to_array) function).
  - If a VARIANT column contains XML, Snowflake recommends explicitly casting the column values to
    ...
    [TO\_XML](/sql-reference/functions/to_xml) function unloads XML-formatted strings
...
- When unloading to files of type `PARQUET`:
  ...
  When Snowflake produces Parquet files, it uses Apache Arrow library 21.1.0. Ensure that your Parquet consumers are compatible with
  this version of the Apache Arrow library.
...
- Failed unload operations:
  ...
  - A failed unload operation can still result in unloaded data files (for example, if the statement is canceled or exceeds its timeout limit).
    For [Partitioned data unloading](/user-guide/data-unload-overview#label-data-unload-overview-partitioned) or if `INCLUDE_QUERY_ID = TRUE`,
    ...
    [file naming pattern](/user-guide/data-unload-overview#label-data-unload-overview-file-names).
...
- To view the status and history of this command’s executions, use [QUERY\_HISTORY view](/sql-reference/account-usage/query_history).
