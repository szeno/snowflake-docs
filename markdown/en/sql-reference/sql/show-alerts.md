...
Lists the [alerts](/user-guide/alerts) for which you have access privileges.
...
See also:
:   [CREATE ALERT](/sql-reference/sql/create-alert) , [ALTER ALERT](/sql-reference/sql/alter-alert), [DROP ALERT](/sql-reference/sql/drop-alert) , [DESCRIBE ALERT](/sql-reference/sql/desc-alert) , [EXECUTE ALERT](/sql-reference/sql/execute-alert)
...
| `runbook` | URL or free-text reference to a runbook for this alert. Returns NULL if not set. |
| `config` | JSON configuration string stored on the alert. Set using [CREATE ALERT … CONFIG](/sql-reference/sql/create-alert) or [ALTER ALERT … SET CONFIG](/sql-reference/sql/alter-alert). Returns NULL if not set. |
| `was_auto_suspended` | Indicates the auto-suspension status of the alert:   - `TRUE`: The alert is currently suspended due to auto-suspension. - `FALSE`: The alert was previously auto-suspended but has been resumed. - `NULL`: The alert has never been auto-suspended. |
...
The `runbook`, `config`, and `was_auto_suspended` columns are available starting with
BCR 2026\_04.
