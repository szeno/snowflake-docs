...
Creates a new [authentication policy](/user-guide/authentication-policies) in the current or specified schema or replaces
...
[ALTER AUTHENTICATION POLICY](/sql-reference/sql/alter-authentication-policy) statement.
...
`name`
:   Specifies the [identifier](/sql-reference/identifiers) for the authentication policy.
...
`SECURITY_INTEGRATIONS = ( 'string_literal' [ , 'string_literal' , ... ] )`
:   A list of security integrations the authentication policy is associated with. This parameter has no effect when `SAML`, `OIDC`, or
...
:   Determines whether a user must enroll in multi-factor authentication. If this value is used, then
    ...
    [DESCRIBE AUTHENTICATION POLICY](/sql-reference/sql/desc-authentication-policy) command and `MFA_ENROLLMENT = 'REQUIRED_SNOWFLAKE_UI_PASSWORD_ONLY'`, then password
...
| OWNERSHIP | Authentication Policy | - Required to execute a [CREATE OR ALTER AUTHENTICATION POLICY](#label-create-or-alter-auth-policy-syntax) statement for an *existing* authentication policy. |
...
- After creating an authentication policy, you must use the [ALTER ACCOUNT](/sql-reference/sql/alter-account) or
  [ALTER USER](/sql-reference/sql/alter-user) command to set it on an account or user before Snowflake enforces the policy.
...
The following attempt to create an authentication policy fails because the CLIENT\_POLICY parameter specifies drivers
that are not permitted by the CLIENT\_TYPES parameter:
...
For more examples, see [Authentication policies](/user-guide/authentication-policies).
