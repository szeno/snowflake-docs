...
[ALTER FILE FORMAT](/sql-reference/sql/alter-file-format) statement.
...
For more information, see [CREATE OR ALTER FILE FORMAT usage notes](#label-create-or-alter-file-format-usage-notes) and [CREATE OR ALTER <object>](/sql-reference/sql/create-or-alter).
...
:   Specifies the format of the input files (for data loading) or output files (for data unloading). Depending on the format type, you can
    ...
    see [Introduction to loading semi-structured data](/user-guide/semistructured-intro).
...
| OWNERSHIP | File format | - Required to execute a [CREATE OR ALTER FILE FORMAT](#label-create-or-alter-file-format-syntax) statement for an *existing* file format.   Note that in a [managed access schema](/user-guide/security-access-control-configure#label-managed-access-schemas), only the schema owner (i.e. the role with the OWNERSHIP privilege on the schema) or a role with the MANAGE GRANTS privilege can grant or revoke privileges on objects in the schema, including future grants. |
...
- Conflicting file format values in a SQL statement produce an error. A conflict occurs when the same option is specified multiple times
  with different values (for example, `...TYPE = 'CSV' ... TYPE = 'JSON'...`).
- Regarding metadata:
