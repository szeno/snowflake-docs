...
Lists the Multi-party Approval policies in your account, including the creation date,
database and schema names, owner, and any available comments.
...
`ON ACCOUNT`
:   Shows the Multi-party Approval policies that have been applied to the current account.
...
| `kind` | Type of policy. Always `MULTI_PARTY_APPROVAL_POLICY`. |
| `owner` | Role that owns the policy. |
| `comment` | Comment for the policy, if any. |
| `owner_role_type` | Type of the role that owns the policy, for example `ROLE` or `DATABASE_ROLE`. |
