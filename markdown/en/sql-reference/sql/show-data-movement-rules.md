...
Lists the [data movement rules](/user-guide/data-movement-policies) for which you have access privileges. The command
returns the creation date, database and schema names, movement type, owner, and any available comments.
...
See also:
:   [CREATE DATA MOVEMENT RULE](/sql-reference/sql/create-data-movement-rule) , [ALTER DATA MOVEMENT RULE](/sql-reference/sql/alter-data-movement-rule) , [DROP DATA MOVEMENT RULE](/sql-reference/sql/drop-data-movement-rule) , [DESCRIBE DATA MOVEMENT RULE](/sql-reference/sql/desc-data-movement-rule)
...
```
SHOW DATA MOVEMENT RULES [ LIKE '<pattern>' ]
                         [ IN
                              {
                                ACCOUNT                  |
...
| `database_name` | VARCHAR | Database in which the rule is stored. |
| `schema_name` | VARCHAR | Schema in which the rule is stored. |
| `movement_type` | VARCHAR | Movement type the rule applies to. |
| `owner` | VARCHAR | Name of the role that owns the rule. |
| `comment` | VARCHAR | Comment for the rule, if any. |
| `owner_role_type` | VARCHAR | Type of the role that owns the rule. |
...
A SHOW command doesn’t require a privilege on the objects it lists. The output includes only the data movement rules that
...
The following example lists the data movement rules in the `PUBLIC` schema of the `mydb` database:
