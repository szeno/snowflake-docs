...
[ALTER STAGE](/sql-reference/sql/alter-stage) statement.

For more information, see [CREATE OR ALTER STAGE usage notes](#label-create-or-alter-stage-usage-notes) and [CREATE OR ALTER <object>](/sql-reference/sql/create-or-alter).
...
:   Specifies the file format for the stage, which can be either:
    ...
    >     well as any other format options, for the data files loaded using this stage. For more details, see [CREATE FILE FORMAT](/sql-reference/sql/create-file-format).
    >
    ...
    >     > - Unloading data into a stage (using [COPY INTO <location>](/sql-reference/sql/copy-into-location)) accommodates `CSV`, `JSON`, or
    >     >   `PARQUET`.
    >
    >     If a file format type is specified, additional format-specific options can be specified. For more details, see
    >     [Format type options ()](#label-create-stage-formattypeoptions) (in this topic).
    >
...
:   Specifies the type of encryption supported for all files stored on the stage. You cannot change the encryption type after you create the stage.
    ...
    :   Specifies the encryption type used.
        ...
        - `SNOWFLAKE_FULL`: Client-side and server-side encryption. The files are encrypted by a client when it uploads them to the internal stage
          using [PUT](/sql-reference/sql/put). Snowflake uses a 128-bit encryption key by default. You can configure a 256-bit key by setting the [CLIENT\_ENCRYPTION\_KEY\_SIZE](/sql-reference/parameters#label-client-encryption-key-size) parameter.
        ...
        - `SNOWFLAKE_SSE`: Server-side encryption only. The files are encrypted when they arrive on the stage by the cloud service
          ...
          [Types of URLs available to access files](/user-guide/unstructured-intro#label-unstructured-data-urls).
...
:   Required only if the storage location is private/protected; not required for public buckets/containers
    ...
    **Amazon S3**
    ...
    >     Snowflake identity and access management (IAM) entity. For more details, see [CREATE STORAGE INTEGRATION](/sql-reference/sql/create-storage-integration).
    >
    >     Note
    >
    ...
    **Microsoft Azure**
    ...
    >     - We highly recommend the use of storage integrations. This option avoids the need to supply cloud storage credentials using
    >       the CREDENTIALS parameter when creating stages or loading data.
    >     - Accessing Azure blob storage in [government regions](/user-guide/intro-regions#label-us-gov-regions)
...
:   Specifies whether Snowflake should automatically refresh the directory table metadata when new or updated
    data files are available on the [internal named stage](/user-guide/data-load-local-file-system-create-stage#label-data-load-local-file-system-table-stage-named-stages).
    ...
    :   Snowflake does not automatically refresh the directory table metadata. You must manually refresh the directory
        table metadata periodically using [ALTER STAGE](/sql-reference/sql/alter-stage) … REFRESH to synchronize the metadata with the current
...
> >         exist in the stage, you must manually refresh the directory table metadata once using [ALTER STAGE](/sql-reference/sql/alter-stage) … REFRESH.
> >
> >     Default: `TRUE`
> >
> > `AUTO_REFRESH = { TRUE | FALSE }`
...
- The association breaks between the stage and any external table that references it.
  ...
  (using CREATE OR REPLACE EXTERNAL TABLE) to reestablish the association. Call the [GET\_DDL](/sql-reference/functions/get_ddl) function to
...
> ```
>
> Create an external stage using a GCS bucket named `load` with a folder path named `encrypted_files` and client-side
> encryption (default encryption type) with the master key to decrypt/encrypt files stored in the bucket:
>
> Copy code
>
