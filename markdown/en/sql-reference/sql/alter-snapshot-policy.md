...
The ALTER SNAPSHOT POLICY command is deprecated. See [ALTER BACKUP POLICY](/sql-reference/sql/alter-backup-policy).

Modifies the properties of a [snapshot](/user-guide/backups) policy. The following changes are supported:
...
:   Unset one of the following properties for the snapshot policy. The property reverts to its default value.
    ...
    For example, you might keep the EXPIRE\_AFTER\_DAYS property when you don’t intend to create new snapshots,
