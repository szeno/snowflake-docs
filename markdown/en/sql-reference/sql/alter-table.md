...
> ```
>
> For details, see [Search optimization actions ()](#label-alter-table-searchoptimizationaction).
...
> Detaches the table from the [DCM project](/user-guide/dcm-projects/dcm-projects-overview) that currently manages it.
> The command removes the association between the table and the DCM project without dropping the table. See [Detach objects from a DCM project](/user-guide/dcm-projects/dcm-projects-use#label-dcm-projects-detach-object) for more information.
...
`ADD [ COLUMN ] [ IF NOT EXISTS ] col_name col_data_type [ AS ( expr ) ]` `[ DEFAULT default_value | AUTOINCREMENT ... ]` `[ inlineConstraint ]` `[ COLLATE 'collation_specification' ]` `[ [ WITH ] MASKING POLICY policy_name ]` `[ [ WITH ] PROJECTION POLICY policy_name ]` `[ [ WITH ] TAG ( tag_name = 'tag_value' [ , tag_name = 'tag_value' , ... ] ) ] [ , ...]`
:   Adds a new column. You can specify a default value, an inline constraint, a [collation specification](/sql-reference/collation#label-collate-clause),
...
For [multi-value tags](/user-guide/object-tagging/multi-value-tags), use `ADD VALUE` and `DROP VALUE` to add or remove individual
string values on a table or column. `SET TAG` replaces the entire value set with a single string.
...
:   Specifies that you want to configure search optimization for specific columns or VARIANT fields (instead of the entire table).
    ...
    - `target` specifies the column, VARIANT field, or an asterisk (\*).
      ...
      To specify a VARIANT field, use [dot or bracket notation](/user-guide/querying-semistructured#label-traversing-semistructured-data) (for example,
    ...
    - `ANALYZER => 'analyzer_name'` specifies the name of the text analyzer, if `search_method`
      ...
      [SEARCH](/sql-reference/functions/search) or [SEARCH\_IP](/sql-reference/functions/search_ip) function, the analyzer
      ...
      - UNICODE\_ANALYZER: Tokenizes based on Unicode segmentation rules that treat spaces and certain
        ...
        See [Using an analyzer to adjust search behavior](/sql-reference/functions/search#label-search-function-adjust-search-behavior).
...
- Dropping a column does not immediately free up the column’s storage space.
  ...
  - The space in each micro-partition is not reclaimed until that micro-partition is re-written. Write
    ...
    1. Use a [CREATE TABLE AS SELECT (CTAS)](/sql-reference/sql/create-table) statement to create a new table that contains
       only the columns of the old table you want to keep.
    2. Set the [DATA\_RETENTION\_TIME\_IN\_DAYS](/sql-reference/parameters#label-data-retention-time-in-days) parameter to `0` for the old table (optional).
...
- Indexes in hybrid tables:
  ...
  - When you use the ALTER TABLE command to add or drop a UNIQUE or
    ...
    table indexes, see [CREATE INDEX](/sql-reference/sql/create-index).
  - Adding a UNIQUE or FOREIGN KEY constraint to a hybrid table is an
    online operation. Snowflake builds the index that backs the constraint
    in the background, and the table remains available for SELECT and DML
    statements while the build runs. The ALTER TABLE statement returns
    before the build finishes, so use
    [SHOW INDEXES](/sql-reference/sql/show-indexes) to check the `status` column for
    `BUILD IN PROGRESS` or `ACTIVE`. Snowflake builds one index at a time
    for a given hybrid table.
  - If rows that are already in the table violate the constraint you’re
    adding, the ALTER TABLE statement still succeeds, but the background
    build fails validation and SHOW INDEXES reports a status of
    `BUILD VALIDATION FAILURE`. The constraint continues to enforce new
    writes while it’s in this state. To recover, drop the constraint,
    correct the offending rows, and add the constraint again. For more
    information, see
    [When existing data violates the constraint](/sql-reference/sql/create-hybrid-table#label-hybrid-table-constraint-validation-failure).
...
[collation specification](/sql-reference/collation#label-collate-clause):
