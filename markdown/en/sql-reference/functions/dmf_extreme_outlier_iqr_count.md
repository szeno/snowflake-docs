...
# EXTREME\_OUTLIER\_IQR\_COUNT (system data metric function)
...
Returns the count of non-NULL, non-NaN values in a column that fall outside the extreme Tukey fences (3 times the interquartile
...
- The function computes the first and third quartiles (Q1, Q3) of the non-NULL, non-NaN values and the interquartile range
  `IQR = Q3 - Q1`, then counts values less than `Q1 - 3 * IQR` or greater than `Q3 + 3 * IQR`. This is a wider fence than
  [OUTLIER\_IQR\_COUNT](/sql-reference/functions/dmf_outlier_iqr_count) uses, so it flags only the most extreme values.
...
Measure the number of extreme outlier values for the transaction\_amount column in a finance table:
