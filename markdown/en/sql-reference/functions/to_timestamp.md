...
[TIMESTAMP\_TYPE\_MAPPING](/sql-reference/parameters#label-timestamp-type-mapping) session parameter. The parameter default is
...
:   [TRY\_TO\_TIMESTAMP / TRY\_TO\_TIMESTAMP\_\*](/sql-reference/functions/try_to_timestamp) ,
    ...
    [AS\_TIMESTAMP\_\*](/sql-reference/functions/as_timestamp) , [IS\_TIMESTAMP\_\*](/sql-reference/functions/is_timestamp) ,

    [TO\_DATE , DATE](/sql-reference/functions/to_date) , [TO\_TIME , TIME](/sql-reference/functions/to_time)
...
`format`
:   Format specifier (only for `string_expr`). For more information, see [Date and time formats in conversion functions](/sql-reference/functions-conversion#label-date-time-format-conversion).

    The default value is the current value of the [TIMESTAMP\_INPUT\_FORMAT](/sql-reference/parameters#label-timestamp-input-format) parameter (default
...
Assume that the [TIMESTAMP\_TZ\_OUTPUT\_FORMAT](/sql-reference/parameters#label-timestamp-tz-output-format) is not set, so the
[TIMESTAMP\_OUTPUT\_FORMAT](/sql-reference/parameters#label-timestamp-output-format) is used and is set to the default
