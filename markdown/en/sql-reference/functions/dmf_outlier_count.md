...
# OUTLIER\_COUNT (system data metric function)
...
Returns the count of non-NULL, non-NaN values in a column that fall outside the asymmetric Tukey fences for the column.
...
```
SNOWFLAKE.CORE.OUTLIER_COUNT(<query>)
```
...
- The function computes the first, second (median), and third quartiles (Q1, Q2, Q3) of the non-NULL, non-NaN values, then counts
  values less than `Q1 - 3 * (Q2 - Q1)` or greater than `Q3 + 3 * (Q3 - Q2)`.
...
Measure the number of outlier values for the order\_total column in a sales table:

Copy code

```
SELECT SNOWFLAKE.CORE.OUTLIER_COUNT(
  SELECT
    order_total
...
```
+---------------------------------------------------------------------------+
| SNOWFLAKE.CORE.OUTLIER_COUNT(SELECT order_total FROM sales.public.orders) |
+---------------------------------------------------------------------------+
| 12                                                                        |
+---------------------------------------------------------------------------+
```
