...
Returns TRUE if its [VARIANT](/sql-reference/data-types-semistructured#label-data-type-variant) argument contains a [string value](/sql-reference/data-types-text#label-character-datatypes).
...
The following examples use the IS\_VARCHAR function.

### Use the IS\_VARCHAR function in a WHERE clause
...
Show the string values in the data by using the IS\_VARCHAR function in a WHERE clause:
...
### Use the IS\_VARCHAR function in a SELECT list
...
Show whether a column contains string values in the data by using the IS\_VARCHAR function in a SELECT list:
