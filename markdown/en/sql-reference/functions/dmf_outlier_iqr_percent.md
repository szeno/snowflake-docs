...
# OUTLIER\_IQR\_PERCENT (system data metric function)
...
Returns the percentage of a column’s values that fall outside the standard Tukey fences (1.5 times the interquartile range) for the
column.
...
- The function computes the first and third quartiles (Q1, Q3) of the non-NULL, non-NaN values and the interquartile range
  `IQR = Q3 - Q1`, then measures the percentage of values less than `Q1 - 1.5 * IQR` or greater than `Q3 + 1.5 * IQR`.
...
Measure the percentage of outlier values for the transaction\_amount column in a finance table:
