# SYSTEM$CREATE\_SEMANTIC\_VIEW\_FROM\_OSSIE\_YAML
...
[Apache Ossie (incubating)](https://github.com/apache/ossie) YAML format.
...
This stored procedure accepts an Ossie YAML document, converts it to the Snowflake semantic model format, and creates the
...
`'ossie_yaml_specification'`
:   The semantic model definition as a YAML string conforming to the Apache Ossie (incubating) Core Metadata Spec.
...
```
Semantic view was successfully created from Ossie YAML.
```

If the Ossie YAML is invalid, the schema isn’t found, column references can’t be resolved, or any other validation
...
## Ossie YAML specification

The top-level fields that Snowflake recognizes in the Ossie YAML are:

| Field | Required | Description |
| --- | --- | --- |
| `name` | Yes | Unique identifier for the semantic model. Becomes the semantic view name. |
| `version` | Yes | Ossie specification version (for example, `"0.1.1"`). Must be a supported version. |
...
| `datasets` | No | Logical datasets (tables/views) that underlie the model. See [Dataset fields](#label-ossie-dataset-fields). |
| `relationships` | No | Foreign-key relationships between datasets. See [Relationship fields](#label-ossie-relationship-fields). |
| `metrics` | No | Model-level aggregate metrics. See [Metric fields](#label-ossie-metric-fields). |
...
| `fields` | No | Row-level field definitions. See [Field classification](#label-ossie-field-classification). |
...
| Ossie field type | Snowflake column type |
| --- | --- |
...
- The `version` field in the Ossie YAML must be a supported version. Currently supported: `0.1.1`.
  Unsupported versions cause the procedure to raise an error listing the supported values.
- The semantic view name is taken from the `name` field in the Ossie YAML. The view is created in the schema
...
- Ossie metadata not natively represented in Snowflake’s semantic view format (`ai_context`, `custom_extensions`, `version`)
...
## Ossie field-to-Snowflake mapping reference

| Ossie construct | Snowflake semantic view construct |
| --- | --- |
| `datasets` | `tables` |
...
### Create a semantic view from a minimal Ossie YAML
