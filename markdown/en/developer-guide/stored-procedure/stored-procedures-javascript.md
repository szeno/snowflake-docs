...
JavaScript variables of type number, string, and [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate) can be bound. (For details about the
...
TIMESTAMP\_LTZ binds an [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate) variable that is created inside the stored procedure.
...
> variable of type [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate). The stored procedure then binds both the original `VARCHAR` and the `TIMESTAMP_LTZ` to an `INSERT` statement. This also demonstrates binding of JavaScript numeric data.
>
> Copy code
>
> ```
...
### Using RESULT\_SCAN to retrieve the result from a stored procedure
