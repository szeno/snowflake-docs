...
- Specify `TABLE(...)` as the procedure’s return type in your [CREATE PROCEDURE](/sql-reference/sql/create-procedure) statement.
  ...
  handler. Column data types will be converted to SQL according to the mapping specified in [SQL-Java Data Type Mappings](/developer-guide/udf-stored-procedure-data-type-mapping#label-sql-java-data-type-mappings).
- Write the handler so that it returns the tabular result in a Snowpark DataFrame.

  For more information about dataframes, see [Working with DataFrames in Snowpark Java](/developer-guide/snowpark/java/working-with-dataframes).
