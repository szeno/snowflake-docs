...
3. Create a new version or patch for the changes in the application package.
   ...
   [automated security scan](/developer-guide/native-apps/security-overview) is initiated.
...
1. To change the upgrade date and time for a custom release directive:
...
[ALTER APPLICATION PACKAGE … RELEASE DIRECTIVE (Legacy)](/sql-reference/sql/alter-application-package-release-directive) command to set the release directive as shown
...
for the account USER\_ACCOUNT.snowflakecomputing.com.
...
This command initiates the upgrade of an installed version or patch of an app using the release directive
...
You can use the [APPLICATION\_STATE view](/sql-reference/data-sharing-usage/application-state-view) in the [Data Sharing Usage](/sql-reference/data-sharing-usage) schema to
...
automated upgrades may take some time depending on multiple factors, including:
...
For example, suppose you updated the default release directive and want to see if all apps have
reached the target version. To find application instances that haven’t yet finished the upgrade, use the query in
...
the app is deployed. For information on upgrade states see [Upgrade states](#label-native-apps-upgrade-process-legacy).
...
Using this view, providers can diagnose issues with specific applications. See [Monitor the state of an upgrade](#label-native-apps-upgrade-status-legacy)
...
You can view the DISABLEMENT\_REASONS column of the [APPLICATION\_STATE view](/sql-reference/data-sharing-usage/application-state-view)
to see the reasons an app is disabled. The following table lists the possible values for the DISABLEMENT\_REASONS
column:
