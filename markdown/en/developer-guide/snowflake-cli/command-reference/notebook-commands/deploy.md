...
The `snow notebook deploy` command uploads local files to a stage and creates a new Notebook object inside your chosen database and schema. Your [project definition file](/developer-guide/snowflake-cli/notebooks/use-notebooks#label-cli-deploy-notebook) should specify the main notebook file and query warehouse. The `--replace` option replaces the specified Notebook object if it already exists.
