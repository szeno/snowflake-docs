...
The `snow dbt describe` command describes a dbt project object on Snowflake. It returns information such as the project name, owner, dbt version and dbt snowflake version, default versions with their names and aliases, and [external access integrations](/developer-guide/external-network-access/creating-using-external-network-access).

For more information, see [DESCRIBE DBT PROJECT](/sql-reference/sql/desc-dbt-project).
