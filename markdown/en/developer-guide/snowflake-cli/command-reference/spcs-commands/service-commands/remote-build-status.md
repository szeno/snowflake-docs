# snow spcs service remote-build-status
...
Displays the current status of a remote build job. Looks up the job in the live service store first, then falls back to the 30-day job history for completed jobs.
...
`--job-name TEXT`
:   Fully-qualified name of the remote build job to look up (e.g. MYDB.PUBLIC.SPCS\_IMAGE\_BUILDER\_JOB\_abc123).
