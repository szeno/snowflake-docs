# snow connection generate-workload-identity-token
...
The `snow connection generate-workload-identity-token` command generates a workload identity token for the current execution environment. Use this token for workload identity authentication when connecting to Snowflake from a workload running in AWS, GCP, Azure, or any OIDC-compliant environment.

You can select the provider with the `--workload-identity-provider` option, or let Snowflake CLI resolve it from the active connection configuration.
