...
- The [com.snowflake.snowpark package](../reference/scala/com/snowflake/snowpark/index.html) contains the main classes for the Snowpark API. To import the names in this package, use:
  ...
  ```
- The [com.snowflake.snowpark.functions object](../reference/scala/com/snowflake/snowpark/functions$.html) defines utility functions (including
  [system-defined functions](/sql-reference-functions)). To import the function names from this object, use:
  ...
  ```
- The [com.snowflake.snowpark.types package](../reference/scala/com/snowflake/snowpark/types/index.html) defines classes and objects that you can use to define schemas for semi-structured
  data.
...
If you used the `run.sh` script to [start the Scala REPL](/developer-guide/snowpark/scala/quickstart-scala-repl), the script already imports names
...
- Any additional JDBC parameters (see [JDBC Driver connection parameter reference](/developer-guide/jdbc/jdbc-parameters) in the JDBC driver documentation) needed to connect
  ...
  To change the logging level (e.g. from `INFO` to `DEBUG`), see [Troubleshooting](/developer-guide/snowpark/scala/troubleshooting#label-snowpark-change-log-level).
- (Optional) `snowpark_request_timeout_in_seconds`: Set this to the maximum number of seconds that the Snowpark library
  ...
  - Waiting for [dependencies to be uploaded to a stage](/developer-guide/snowpark/scala/creating-udfs#label-snowpark-udf-inline-dependencies).
  - Waiting for an [asynchronous action](/developer-guide/snowpark/scala/working-with-dataframes#label-snowpark-dataframe-action-method-async) to complete.
