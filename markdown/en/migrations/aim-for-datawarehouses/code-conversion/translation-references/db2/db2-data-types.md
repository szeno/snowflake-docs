...
| TIME | TIME |  |
| TIMESTAMP | TIMESTAMP |  |
| XML | VARIANT | [SSC-EWI-0036](../../issues-and-troubleshooting/conversion-issues/generalEWI#ssc-ewi-0036) |
| BOOLEAN | BOOLEAN |  |
...
DB2 `DECFLOAT` columns are transformed to Snowflake’s native `DECFLOAT` data type in table column definitions and `CAST` expressions.
...
When inserting data into `DECFLOAT` columns, `CAST` expressions are automatically added to ensure proper data type handling:
...
1. [SSC-FDM-DB0002](../../issues-and-troubleshooting/functional-difference/db2FDM#ssc-fdm-db0002): DECFLOAT is not supported in this context.
