# Oracle - Oracle Built-in Data Types
...
JSON data insertions are not being correctly handled.

**2. JSON objects manipulation**

The usages of JSON objects (columns, variables, or parameters) are not correctly converted. Check the [Recommendations](#json-data-type) section for workarounds
...
##### Snowflake CREATE OR REPLACE TABLE raw\_table
...
When specifying a scale greater than the maximum allowed in Snowflake (37) it is being changed to 18. To get more information about this please go to the [SSC-FDM-0006](../../../../issues-and-troubleshooting/functional-difference/generalFDM#ssc-fdm-0006) documentation.

##### 2. Negative scale

Snowflake does not allow negative scale, so it is being removed. This could cause functional inequivalence. To get more information about this issue please go to the [SSC-EWI-0R0092](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0092) documentation.
...
1. [SSC-EWI-OR0092](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0092) Number datatype negative scale was removed from output.
...
3. [SSC-FDM-OR0010](../../../../issues-and-troubleshooting/functional-difference/oracleFDM#ssc-fdm-or0010) Number datatype smaller precision was increased to match scale
...
#### 1. The BINARY\_DOUBLE data type is not supported by Snowflake
...
#### 1. The BINARY\_FLOAT data type is not supported by Snowflake
...
**Preview Feature:** When the `--UseIntervalDatatype` preview flag in the SnowConvert AI Desktop application is enabled, Oracle `INTERVAL DAY TO SECOND` columns are preserved as native Snowflake `INTERVAL DAY TO SECOND` types. See the [Interval Data Types](../../../general/interval-data-types) translation reference for complete transformation details.
...
**Preview Feature:** When the `--UseIntervalDatatype` preview flag in the SnowConvert AI Desktop application is enabled, Oracle `INTERVAL YEAR TO MONTH` columns are preserved as native Snowflake `INTERVAL YEAR TO MONTH` types. See the [Interval Data Types](../../../general/interval-data-types) translation reference for complete transformation details.
...
When operating timestamps with local timezone data types, Oracle converts the timestamps to the default timezone of the database. To emulate this behavior in Snowflake, the TIMESTAMP\_TYPE\_MAPPING session parameter should be set to ‘TIMESTAMP\_LTZ’.
...
In Oracle, some arithmetic operations could be performed between DateTime types, like addition, subtraction, multiplication, and division. Currently, some cases of addition and subtraction can be resolved. These cases are explained below.
...
| **Number** | Date + Interval day | Timestamp + Interval day | Number + Number | INVALID | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Number + Float |
| **Interval** | Date + Interval IntervalUnit | Timestamp + Interval IntervalUnit | INVALID | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Unknown + Interval IntervalUnit | INVALID |
| **Unknown** | DATEADD\_UDF | DATEADD\_UDF | Unknown + Number | Unknown + Interval IntervalUnit | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) |
| **Float** | DATEADD\_UDF | DATEADD\_UDF | Float + Number | INVALID | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Float + Float |
...
By default, Snow Convert migrates operations of type Date/Timestamp + Interval to the native Snowflake operations, but in some cases may be useful to use [UDF](../../functions/custom_udfs#dateadd-udf-interval) instead. For further details, see [Interval UDFs vs. Snowflake native interval operation](#interval-udfs-vs-snowflake-native-interval-operation).
...
For those cases where there is an addition operation between a date or timestamp type and an unknown type, a user-defined function (UDF) is added. See the [DATEADD\_UDF implementation](../../functions/custom_udfs#dateadd-udf-interval) for details. The UDF is located in the UDFs folder. For example:
...
| **Number** | INVALID | INVALID | Number - Number | INVALID | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Number - Float |
| **Interval** | INVALID | INVALID | INVALID | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Unknown - Interval IntervalUnit | NOT SUPPORTED IN ORACLE |
| **Unknown** | DATEDIFF\_UDF | DATEDIFF\_UDF | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Unknown - Interval IntervalUnit | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) |
| **Float** | DATEDIFF\_UDF | DATEDIFF\_UDF | Float - Number | NOT SUPPORTED IN ORACLE | [SSC-EWI-OR0036](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0036) | Float - Float |
...
By default, Snow Convert migrates operations of type Date/Timestamp + Interval to the native Snowflake operations, but in some cases may be useful to use [UDF](../../functions/custom_udfs#datediff-udf-interval) instead. For further details, see [Interval UDFs vs. Snowflake native interval operation](#interval-udfs-vs-snowflake-native-interval-operation).
...
For more information, see the [Oracle NLS\_DATE\_FORMAT documentation](https://docs.oracle.com/en/database/oracle/oracle-database/21/refrn/NLS_DATE_FORMAT.html#GUID-FC23EEEE-AA9F-4B3C-8CBB-888C9C0CA27F).

#### TIMESTAMP\_DIFF\_UDF

The subtractions between timestamp types and dates with a timestamp and vice versa; are resolved by inserting the TIMESTAMP\_DIFF\_UDF user-defined function, (see the [TIMESTAMP\_DIFF\_UDF implementation](../../functions/custom_udfs#timestamp-diff-udf)). For example
...
For those cases where there is an addition operation between a date or timestamp type and an unknown type, a user-defined function (UDF) is added. See the [DATEDIFF\_UDF implementation](../../functions/custom_udfs#datediff-udf), which could be edited to perform what is required. The UDF is located in the UDFs folder. For example:
...
The TIMESTAMP\_DIFF\_UDF must be improved to be able to specify the return type. It means adding a third parameter where it is possible to
...
The following table shows a comparison between the [DATEADD\_UDF INTERVAL](../../functions/custom_udfs#dateadd-udf-interval) and [DATEDIFF\_UDF INTERVAL](../../functions/custom_udfs#datediff-udf-interval) vs the [Snowflake native operation](https://docs.snowflake.com/en/sql-reference/data-types-datetime.html#interval-constants) for interval arithmetic.
...
## BINARY\_INTEGER Data Type

This data type is identical to the [PLS\_INTEGER](#pls_integer-data-type) data type.

## PLS\_INTEGER Data Type
...
Some of these subtypes are currently not recognized, so they are converted to `VARIANT` and considered user-defined types. There is already a work item to fix the issue.
...
#### PLS\_INTEGER usage in procedural blocks
...
[![](/static/images/migrations/sc-assets/image(106)(1).png)](/static/images/migrations/sc-assets/image(106)(1).png)

##### Snowflake

[![](/static/images/migrations/sc-assets/image(198)(1).png)](/static/images/migrations/sc-assets/image(198)(1).png)
...
1. [SSC-FDM-OR0015](../../../../issues-and-troubleshooting/functional-difference/oracleFDM#ssc-fdm-or0015): LENGTHB transformed to OCTET\_LENGTH.
...
Even though, the syntax is parsed and transformed using the [ANSI, DB2, and SQL/DS Data Types.](README#ansi-data-types)
...
1. [SSC-EWI-OR0105](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0105): Additional work is needed for BFILE column usage. BUILD\_STAGE\_URL function is a recommended workaround.
...
This example **is not a translation**, it is only used to show the functional equivalence between Oracle `BLOB` and Snowflake `BINARY`
...
##### 2. Empty value with EMPTY\_BLOB
...
##### 3. No access to the DBMS\_LOB built-in package
...
##### 2. Empty value with EMPTY\_CLOB
