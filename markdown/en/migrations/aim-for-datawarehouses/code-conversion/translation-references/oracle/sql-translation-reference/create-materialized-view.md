# Oracle - Create Materialized Views

Translation reference to convert Oracle Materialized View to Snowflake Dynamic Table

## Description

Oracle Materialized Views are transformed into Snowflake Dynamic Tables. To properly configure Dynamic Tables, two essential parameters must be defined: TARGET\_LAG and WAREHOUSE. If these parameters are left unspecified in the configuration options, preassigned values will be used during the conversion, as demonstrated in the example below.
...
The dynamic table’s default refresh mode is **AUTO**.
