# SQL Server-Azure Synapse - CONTINUE HANDLER
...
However, when migrating code from other database systems that use CONTINUE HANDLERs, these constructs are transformed into equivalent Snowflake Scripting exception handling mechanisms.
...
When migrating stored procedures from DB2 or Teradata that contain CONTINUE HANDLER declarations, they are transformed into Snowflake-compatible exception handling.
...
CONTINUE HANDLER behavior is emulated in Snowflake as best as possible, but there are limitations:
...
- [TRY CATCH Translation Reference](transact-create-procedure-snow-script#try-catch)
