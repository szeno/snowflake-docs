...
SQL Server Scalar User Defined Functions are supported to be translated directly to [Snowflake Scripting UDFs](/developer-guide/udf/sql/udf-sql-procedural-functions) when they meet specific criteria.  
...
1. [SSC-FDM-TS0012](../../issues-and-troubleshooting/functional-difference/sqlServerFDM#ssc-fdm-ts0012): Information for the expression was not found. CAST to STRING used
...
All the code samples on this page have not been implemented yet. They should be interpreted as a reference for how each scenario should be translated to Snowflake. These translations may change in the future.Some parts in the output code are omitted for clarity reasons.
...
For now, only `SQL` and `JavaScript` are supported as target languages.
...
This is all aggravated by nesting and complex querying that can be found on input code. That’s why a specific [EWI](../../issues-and-troubleshooting/conversion-issues/generalEWI#ssc-ewi-0068) is added when these patterns are found.
...
Common table expressions will be kept as in the original code, and they are going to be concatenated with the generated ones. All the original `COMMON TABLE EXPRESSION` names are identified first to avoid generating duplicated names.
...
2. Calls to user-defined functions inside procedures, should be preceded by the `CALL` keyword.
...
1. [SSC-EWI-0067](../../issues-and-troubleshooting/conversion-issues/generalEWI#ssc-ewi-0067): UDF was transformed to Snowflake procedure, calling procedures inside a query is not supported.
2. [SSC-EWI-0068](../../issues-and-troubleshooting/conversion-issues/generalEWI#ssc-ewi-0068): User defined function was transformed to a Snowflake procedure.
3. [SSC-EWI-0073](../../issues-and-troubleshooting/conversion-issues/generalEWI#ssc-ewi-0073): Pending Functional Equivalence Review.
...
SQL Server Scalar User Defined Functions can be translated directly to **Snowflake Scripting UDFs** (SnowScript UDFs) when they meet specific criteria, instead of converting all functions to Stored Procedures.
...
Each SQL Server function is analyzed and the appropriate Snowflake target is automatically determined. A function becomes a SnowScript UDF when it contains **only** procedural logic without data access operations.
...
**UDFs containing the following elements will not be translated into SnowScripting UDFs, as these features are unsupported in SnowScripting UDFs:**
