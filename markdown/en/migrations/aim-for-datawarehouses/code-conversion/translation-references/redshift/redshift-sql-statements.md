# Redshift - SQL Statements

Translation reference for all the supported statements for Redshift.
...
The connection limit clause is removed since the connection concurrency in Snowflake is managed by warehouse. For more information, see the [Snowflake MAX\_CONCURRENCY\_LEVEL parameter](https://docs.snowflake.com/en/sql-reference/parameters#label-max-concurrency-level).
...
Currently `CREATE EXTERNAL TABLES` are being transformed to regular tables, that implies additional effort because data stored in external RedShift tables must be transferred to the Snowflake database.
...
1. [SSC-FDM-0004](../../issues-and-troubleshooting/functional-difference/generalFDM#ssc-fdm-0004): External table translated to regular table
...
- [Sortkey](redshift-sql-statements-create-table#sortkey)
- [DistKey](redshift-sql-statements-create-table#distkey)
- [DistStyle](redshift-sql-statements-create-table#diststyle)
...
- [SSC-FDM-0031](../../issues-and-troubleshooting/functional-difference/generalFDM#ssc-fdm-0031): Dynamic Table required parameters set by default
...
Within the current scope, the Python language for `CREATE FUNCTION` statements is not supported. Consequently, the language `plpythonu` will be flagged with an EWI (SSC-EWI-0073), and its body could appear with parsing errors.
...
- [SSC-EWI-RS0003](../../issues-and-troubleshooting/conversion-issues/redshiftEWI#ssc-ewi-rs0003): With no schema binding statement is not supported in Snowflake.
...
2. [SSC-EWI-RS0008](../../issues-and-troubleshooting/conversion-issues/redshiftEWI#ssc-ewi-rs0008): Materialized view is transformed into a dynamic table, and the DELETE statement cannot be used on dynamic tables in Snowflake.
...
1. [SSC-EWI-RS0009](../../issues-and-troubleshooting/conversion-issues/redshiftEWI#ssc-ewi-rs0009): Semantic information not found for the source table.
2. [SSC-FDM-RS0005](../../issues-and-troubleshooting/functional-difference/redshiftFDM#ssc-fdm-rs0005): Redshift MERGE rejects duplicate source rows. Snowflake allows them, which may produce different results.
