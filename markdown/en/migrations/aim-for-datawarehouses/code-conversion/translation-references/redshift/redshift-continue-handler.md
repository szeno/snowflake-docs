...
However, when migrating code from database systems that use CONTINUE HANDLERs (such as DB2, Teradata, or other systems), these constructs are transformed into equivalent Snowflake Scripting exception handling mechanisms.
...
When migrating stored procedures from systems with CONTINUE HANDLER to Snowflake via Redshift, they are transformed into Snowflake-compatible exception handling.
