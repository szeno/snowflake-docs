...
**Message** Pandas < **pandas.core.frame.DataFrame.sort\_index** > has a partial mapping with a few scenarios not supported in Snowpark.
...
3. **Post-process with native pandas**: If the parameter is critical, collect the result using `.to_pandas()` and apply the operation with native pandas:
   ...
   result = df.to\_pandas().sort\_index(key=value)
