...
**Message** Pandas < **pandas.core.reshape.merge.merge\_asof** > has a partial mapping with a few scenarios not supported in Snowpark.
...
2. **Use `.to_pandas()` for full compatibility**: If you need these parameters, convert to native pandas first:
   ...
   result = native\_df.merge\_asof(…) # Use all parameters
