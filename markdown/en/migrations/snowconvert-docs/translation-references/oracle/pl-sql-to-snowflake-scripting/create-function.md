...
Translating Oracle PL/SQL User Defined Functions directly to **Snowflake Scripting UDFs** (SnowScript UDFs) when they meet specific criteria is supported.
...
Each Oracle function is analyzed and the appropriate Snowflake target automatically determined. A function becomes a SnowScript UDF when it contains **only** procedural logic without data access operations.
