...
| SYS.URIFACTORY.GETURI(‘[HTTP://LOCALHOST’).GETURL(](HTTP://LOCALHOST').GETURL()) |
| --- |
| <http://localhost/> |
...
Subprograms of built-in packages are not transformed. Most of the functionality of URI types is not currently supported by Snowflake.
...
1. [SSC-EWI-OR0007](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0007): Create Type Not Supported in Snowflake.
...
The functions for manipulating and querying XML such as XMLTYPE() are not being recognized nor transformed.
...
2. [SSC-EWI-OR0016:](../../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0016) XML is not supported.
...
| --- |
| <?xml version=”1.0”?>¶ <FIRST\_NAME>Shelley</FIRST\_NAME>¶ |
...
DBURIType is parsed and converted as Custom Data Type or as not supported type if it uses the prefix SYS, there is a work item to fix this issue
...
HTTPURIType is parsed and converted as Custom Data Type or as not supported type if it uses the prefix SYS, there is a work item to fix this issue
...
XDBURIType is parsed and converted as Custom Data Type or as not supported type if it uses the prefix SYS, there is a work item to fix this issue
