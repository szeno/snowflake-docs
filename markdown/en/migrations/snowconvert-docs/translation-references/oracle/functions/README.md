...
| INITCAP | INITCAP |  |
| INSTR | POSITION  REGEXP\_INSTR | Parameter order is inverted. With ***‘occurrence’***, `REGEXP_INSTR` is used. Position = -1 is automatically translated. Positions < -1 emit [SSC-EWI-OR0020](../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0020). Also applies to INSTRB, INSTRC, INSTR2, INSTR4. |
...
| JSON\_VALUE | [*JSON\_VALUE\_UDF*](./custom_udfs#json_value-udf) |  |
...
| SOUNDEX | SOUNDEX |  |
| SQRT | SQRT |  |
| STANDARD\_HASH | SHA1  SHA2  MD5 | Converted based on the algorithm parameter: default/`'SHA1'` → `SHA1`, `'SHA256'`/`'SHA384'`/`'SHA512'` → `SHA2(expr, bits)`, `'MD5'` → `MD5`. A [warning (SSC-FDM-OR0032)](../../../issues-and-troubleshooting/functional-difference/oracleFDM#ssc-fdm-or0032) is emitted when the input is a non-string parameter. Dynamic algorithm parameters emit [SSC-EWI-OR0138](../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0138). |
...
1. [SSC-EWI-OR0045](../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0045): Cast type L and FML are not supported.
2. [SSC-EWI-OR0050](../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0050): Input Expression is out of the range.
3. [SSC-EWI-OR0053](../../../issues-and-troubleshooting/conversion-issues/oracleEWI#ssc-ewi-or0053): Incorrect input format.
