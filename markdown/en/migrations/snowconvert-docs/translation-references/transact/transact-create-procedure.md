# SQL Server-Azure Synapse - Procedures
...
The supported statements are automatically translated and make use of an EXEC helper. This helper provides access and update capabilities to many variables that simulate how the execution of these statements would be in their native environment.
...
The RAISERROR executes the *UPDATE\_ERROR\_VARS\_UDF* to store the value of the error message, severity and state as environment variables, in case they need to be used by calling any of the ERROR built in functions. Finally, the error message is thrown with the same format as SQL Server does.
...
All the usages are identified to generate a helper that emulates the original behavior. Example:
...
The code is modified slightly due to the `INSERT INTO [Table] EXEC(...)` Statement not being supported in Snowflake this allows us to replicate the behavior by adding a few lines of code:
...
Since `CURSORS` are not supported in Snowflake, their functionality is mapped to a `JavaScript` helper that emulates the original behavior in the target platform. Example:
