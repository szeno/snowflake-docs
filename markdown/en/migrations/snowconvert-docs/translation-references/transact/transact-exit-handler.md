# SQL Server-Azure Synapse - EXIT HANDLER
...
However, when migrating code from other database systems that use EXIT HANDLERs, these constructs are transformed into equivalent Snowflake Scripting exception handling mechanisms.
...
When migrating stored procedures from DB2 or Teradata that contain EXIT HANDLER declarations, they are transformed into Snowflake-compatible exception handling.
...
EXIT HANDLER patterns are transformed to Snowflake EXCEPTION blocks, which provide equivalent exit behavior:
