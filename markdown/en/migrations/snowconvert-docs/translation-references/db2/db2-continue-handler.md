...
When migrating from DB2 to Snowflake, CONTINUE HANDLER declarations are transformed into equivalent Snowflake Scripting exception handling using EXCEPTION blocks with appropriate logic to continue execution.
...
2. [SSC-FDM-0027](../../issues-and-troubleshooting/functional-difference/generalFDM#ssc-fdm-0027): REMOVED NEXT STATEMENT, NOT APPLICABLE IN SNOWFLAKE (applies to FROM clause RETURN DATA UNTIL statements)

### SQLSTATE Mapping

Not all DB2 SQLSTATE codes have direct equivalents in Snowflake. Best-effort mapping is performed:
...
DB2 allows declaring both CONTINUE and EXIT handlers in the same procedure block. However, Snowflake Scripting does not support mixing CONTINUE and EXIT handlers in the same EXCEPTION block. When this pattern is encountered, separate EXCEPTION blocks are generated with an EWI warning.
