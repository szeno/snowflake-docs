...
When migrating from DB2 to Snowflake, EXIT HANDLER declarations are transformed into equivalent Snowflake Scripting exception handling using EXCEPTION blocks with `WHEN OTHER EXIT THEN` or specific exception types.
...
See the [CONTINUE HANDLER documentation](db2-continue-handler#mixed-continue-and-exit-handlers) for detailed examples of this limitation.
