...
A metric version pins the judge model and the prompt, rubric, and thresholds used to produce a SQL correctness score. Pinning a version makes scoring consistent from run to run.
...
- **Evaluation credits** for the [AI\_COMPLETE](/sql-reference/functions/ai_complete) function calls used to compute the `sql_correctness` metric.
