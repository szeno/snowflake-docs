...
A shared resource-level budget lets you set a credit spending limit for a subset of users on a Snowflake CoWork
...
team) identified by a tag. This allows multiple teams to share the same Snowflake CoWork object while
maintaining independent spending limits per team.

To use a shared resource-level budget, you create a budget, add the Snowflake CoWork object as a “shared
...
Shared resource-level budgets use a user-tag-based attribution model. Instead of tagging the Snowflake CoWork
...
2. You add the Snowflake CoWork object as a shared resource on the budget.
...
Snowflake CoWork object. This is different from resource-level budgets, where the tag is applied directly
to the object.

## Set up a shared resource-level budget

Follow these steps to create a budget for a subset of users on a shared Snowflake CoWork object.
...
### Step 4: Add the Snowflake CoWork object as a shared resource

Add the Snowflake CoWork object to the budget as a shared resource. This tells Snowflake which resource to
...
on the Snowflake CoWork object against the `my_budget` budget with a 500-credit monthly limit.
...
CALL budgets_db.budgets_schema.my_budget!ADD_CUSTOM_ACTION(
  ...
  ARRAY_CONSTRUCT('SI_1', 'finance_dept', 80),
  'ACTUAL',
  80
);
...
Snowflake CoWork object. Snowflake recommends creating a dedicated role per user group to support this pattern:
...
AS
BEGIN
  EXECUTE IMMEDIATE 'REVOKE ROLE si_' || dept_name || '_role FROM ROLE ' || dept_name || '_role';
...
revokes the dedicated role, which removes access to the Snowflake CoWork object for all users in that group.
...
-- Users tagged with finance_dept can no longer access the SI object
```
...
sp_reinstate_group_access (si_name, power_user_role)
...
CREATE OR REPLACE PROCEDURE budgets_db.budgets_schema.sp_reinstate_group_access(
  si_name STRING, power_user_role STRING
)
RETURNS STRING
LANGUAGE SQL
AS
BEGIN
  EXECUTE IMMEDIATE 'GRANT ROLE si_' || si_name || '_role TO ROLE ' || power_user_role || '_role';
...
View credit consumption per user on the shared Snowflake CoWork object using the budget’s usage reporting
method:
...
| Column | Description |
| --- | --- |
| SERVICE\_TYPE | The service category (SNOWFLAKE\_INTELLIGENCE) |
| ENTITY\_TYPE | The object type (SNOWFLAKE INTELLIGENCE) |
| ENTITY\_ID | The internal identifier of the Snowflake CoWork object |
| NAME | The name of the Snowflake CoWork object |
...
When a user is subject to multiple budgets — for example, both a resource-level budget on the Snowflake CoWork
...
- A resource-level budget on `SI_1` with a 1,000-credit limit and a block action at 100%.
- A shared resource-level budget for the finance department on Snowflake CoWork with a 500-credit limit and a
  block action at 100%.

If the finance department reaches 500 credits (100% of their shared budget) before the overall SI object
reaches 1,000 credits, the shared budget’s block action triggers first and revokes access for the finance
department. Other teams that use `SI_1` continue to have access until the resource-level budget threshold is
reached.

Conversely, if the overall SI object reaches 1,000 credits before the finance department reaches 500 credits,
...
The following limitations apply to shared resource-level budgets for Snowflake CoWork:
