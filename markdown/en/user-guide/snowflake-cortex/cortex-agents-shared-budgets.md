...
Shared resource-level budgets set a spending limit for a tagged group of users on a specific agent. To apply the same monthly or daily credit limit to each user, across Cortex Agents and other AI domains, and optionally block users who reach that limit, use [per-user quotas](/user-guide/budgets/per-user-quotas).
