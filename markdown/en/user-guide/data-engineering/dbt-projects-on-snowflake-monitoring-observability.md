...
Execution results are stored at
`snow://dbt/<database>.<schema>.<project>/results/query_id_<query_id>/`. The directory contains the following standalone files, when
present in the execution’s target and log directories, and the results archive:

| Path relative to the results directory | Contents |
| --- | --- |
| `target/manifest.json` | Project resources and their relationships, used for state comparison and defer. |
| `target/semantic_manifest.json` | Semantic definitions generated by dbt. |
| `target/run_results.json` | Results of a dbt command, used by `result:` selectors. |
| `target/sources.json` | Source freshness results, used by `source_status:` selectors. |
| `logs/dbt.log` | Only the current execution’s dbt log, not a cumulative log or the entire logs directory. |
| `dbt_artifacts.zip` | The full contents of the target and logs directories, including compiled SQL and files not uploaded separately. Archived dbt logs are scoped to the current execution. |

Expand

Show lessSee more

Snowflake uploads only the standalone files listed in the preceding table from the execution’s `target` and `logs` directories. Not every command generates
every file. For example, `compile` doesn’t generate `sources.json`, but if `sources.json` already exists in the live version’s `target`
directory, it will still appear in the query’s artifacts. Files might be unavailable if an execution fails before uploading
its results.

`SYSTEM$LOCATE_DBT_ARTIFACTS` returns the results directory’s location, while `SYSTEM$LOCATE_DBT_ARCHIVE` returns the ZIP file’s location.
These functions locate files. They don’t download or generate them. Use `LIST` to inspect the returned location or `GET` to download files.
The `SYSTEM$DBT_GET_LAST_*_RUN_TARGET` functions instead search execution history and return a matching execution’s target location.

When used in `IMPORTS`, the `SYSTEM$DBT_GET_LAST_*_RUN_TARGET` functions import standalone target artifacts.
`SYSTEM$LOCATE_DBT_ARTIFACTS` imports the results directory, including `dbt_artifacts.zip`, but doesn’t extract the ZIP file.
`SYSTEM$LOCATE_DBT_ARCHIVE` extracts the archive. For state paths and examples, see
.
For a source freshness execution and retrieval example, see
