...
![Stream offset example](/static/images/table-streams-offset.png)
...
The delta records returned by streams in a transaction are the range from the current position of the stream until the transaction start
...
1. The METADATA$ROW\_ID depends on the stream’s source object.
   ...
   rows, but a stream `stream_view` on view `view1` is not guaranteed to produce the same METADATA$ROW\_IDs as `stream1`, even if `view1` is
...
Standard:
:   Supported for streams on standard tables, dynamic tables, Snowflake-managed Apache Iceberg™ tables, externally managed V3 Apache Iceberg™ tables, directory tables, and views. A standard (i.e. delta) stream tracks all DML
...
:   Supported for streams on standard tables, dynamic tables, Snowflake-managed Apache Iceberg™ tables, or views. An append-only stream exclusively tracks row
    ...
    Creating append-only streams in a target account using a secondary object as the source is not supported.
...
![Streams Example](/static/images/data-pipeline-streams.png)
...
determined by the [MAX\_DATA\_EXTENSION\_TIME\_IN\_DAYS](/sql-reference/parameters#label-max-data-extension-time-in-days) parameter value. Once the
stream is consumed, the extended data retention period reverts to the table’s default.

The following table shows examples of [DATA\_RETENTION\_TIME\_IN\_DAYS](/sql-reference/parameters#label-data-retention-time-in-days) and
