...
- Snowflake does not support using the [Time Travel feature](/user-guide/data-time-travel) to query materialized views at
  [a point in the past](/user-guide/data-time-travel#label-time-travel-querying) (e.g. using the [AT clause](/sql-reference/constructs/at-before)
  when querying a materialized view).

  However, you can use Time Travel to
  [clone a database or schema containing a materialized view at a point in the past](/user-guide/data-time-travel#label-time-travel-cloning). For
...
:   As a user with the [proper privileges](/user-guide/cost-access-control), In the navigation menu, select **Admin** » **Cost management**, and then select **Consumption**.
    ...
    The credit costs are tracked in a Snowflake-provided virtual warehouse named [![Snowflake logo in blue (no text)](/static/images/logo-snowflake-sans-text.png)](/static/images/logo-snowflake-sans-text.png) **MATERIALIZED\_VIEW\_MAINTENANCE**.
...
credit usage for the Snowflake-provided warehouses, including the [![Snowflake logo in blue (no text)](/static/images/logo-snowflake-sans-text.png)](/static/images/logo-snowflake-sans-text.png) **MATERIALIZED\_VIEW\_MAINTENANCE** warehouse.
...
- The storage costs depend on the [data retention period](/user-guide/data-time-travel#label-time-travel-data-retention-period) for the materialized
  views, which is determined by the [DATA\_RETENTION\_TIME\_IN\_DAYS](/sql-reference/parameters#label-data-retention-time-in-days) parameter. Materialized
