...
![Diagram illustrating the serverless task compute model.](/static/images/tasks-serverless-hourly.svg)
...
![Diagram showing a serverless task scheduled daily between midnight and 2 a.m.](/static/images/tasks-serverless-mornings.svg)
...
The following example creates a task that runs every Sunday at 3:07 a.m., using the Americas/Los\_Angeles time zone:
...
The [SUSPEND\_TASK\_AFTER\_NUM\_FAILURES](/sql-reference/parameters#label-suspend-task-after-num-failures) parameter can also be set at the account, database, or schema level.
...
:   Snowflake bills your account based on compute resource usage. Charges are calculated based on your total usage of the resources,
    ...
    runtime. For more information, see [Serverless credit usage](/user-guide/cost-understanding-compute#label-serverless-credit-usage) or [Query: Total serverless task cost](/user-guide/cost-exploring-compute#label-cost-explore-query-task).
...
![A diagram of a task, including a 5 second queuing time and a 7 second run time. The task is scheduled to run every 15 seconds.](/static/images/task-duration.svg)
...
When both [STATEMENT\_TIMEOUT\_IN\_SECONDS](/sql-reference/parameters#label-statement-timeout-in-seconds) and [USER\_TASK\_TIMEOUT\_MS](/sql-reference/parameters#label-user-task-timeout-ms) are set, the timeout is the lowest non-zero value of the two parameters.
