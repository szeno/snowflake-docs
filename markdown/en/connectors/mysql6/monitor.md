...
- [Viewing general information about the connector](#label-mysql-connector-monitoring-general-info-6)
- [Viewing data sources](#label-mysql-connector-monitoring-data-sources-6)
- [Viewing the replication state of data sources](#label-mysql-connector-monitoring-data-sources-state-6)
- [Viewing the replication state of source tables](#label-mysql-connector-monitoring-replication-state-6)
- [Viewing table schema version history](#label-mysql-connector-monitoring-schema-version-history-6)
- [Viewing connector metrics](#label-mysql-connector-monitoring-connector-stats-6)
- [Viewing aggregated connector metrics](#label-mysql-connector-monitoring-connector-aggregated-stats-6)
- [Viewing experimental views](#label-mysql-connector-examining-experimental-views)
- [Viewing the connector audit log view](#label-mysql-connector-audit-log-6)
- [Viewing the agent audit log view](#label-mysql-connector-agent-audit-log-6)
- [Viewing the connector logs](#label-mysql-connector-viewing-connector-logs-6)
- [Viewing the agent logs](#label-mysql-connector-viewing-agent-logs-6)
...
| --- | --- |
| alertingLogsView | If you [enabled email notifications](/connectors/mysql6/email-notifications#label-mysql-connector-enable-email-notifications), this specifies the name of [the view that provides access to the event table](/connectors/mysql6/email-notifications#label-mysql-connector-create-logs-view). |
| alertingNotificationIntegration | If you [enabled email notifications](/connectors/mysql6/email-notifications#label-mysql-connector-enable-email-notifications), this specifies the name of the notification integration object used for email notifications. |
| alertingRecipients | If you [enabled email notifications](/connectors/mysql6/email-notifications#label-mysql-connector-enable-email-notifications), this specifies the list of email addresses (separated by commas) that can receive email notifications from the connector. |
| alertingSchedule | If you [enabled email notifications](/connectors/mysql6/email-notifications#label-mysql-connector-enable-email-notifications), this specifies the schedule or frequency at which the connector should check for errors and send a notification. |
...
| REPLICATION\_PHASE | VARCHAR | Current replication phase. Possible values:   - `SCHEMA_INTROSPECTION` - `INITIAL_LOAD` - `INCREMENTAL_LOAD`   For descriptions of each status, see [Understanding replication phases](#label-mysql-connector-monitoring-replication-phase-descriptions-6). |
...
You can start FAILED replications from the beginning by removing table from replication and adding it again as described in [Configuring replication for the Snowflake Connector for MySQL](/connectors/mysql6/configure-replication#label-mysql-connector-configure-replication-6).
...
To view the connector logs, query the event table that you created while setting up the connector [log view](/connectors/mysql6/install-snowsight#label-mysql-connector-configure-logging-6).
...
If required, and after completing these procedures, review the steps in [Troubleshooting the Snowflake Connector for MySQL](/connectors/mysql6/troubleshoot).
