...
To use an existing database as a destination database, the Snowflake Connector for PostgreSQL requires the [CREATE SCHEMA](/sql-reference/sql/create-schema) permission on that database. The connector is the owner of the schemas and tables containing ingested PostgreSQL data.
...
1. [Add a data source](#label-postgres-connector-add-data-source-6).
2. Ensure the agent is stopped.
3. [Configure the agent connection to the new data source](/connectors/postgres6/install-agent#label-postgresql-connector-prepare-agent-config-6).
4. [Run the Docker container of the agent](/connectors/postgres6/install-agent#label-postgres-connector-run-the-docker-container-6).
...
If some tables were recently removed ([Remove a table from replication](#label-postgres-postgres-connector-remove-source-table-6)), it might not be possible to add them yet.
...
To check current schedule, see [Viewing data sources](/connectors/postgres6/monitor#label-postgres-connector-monitoring-data-sources-6).
...
After completing these procedures, follow the steps in [Viewing PostgreSQL data in Snowflake](/connectors/postgres6/view-data)
